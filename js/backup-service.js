...
} from './backup.js?v=480';
import { downloadBlob } from './download.js?v=480';
import { ValidationError } from './validators.js?v=480';
import { openModal, closeModal } from './modal.js?v=480';
import { escapeHtml, showToast } from './utils.js?v=480';
import {
  ...
  isNativeApp,
} from './backup-folder-bridge.js?v=480';
import {
...
function installDbChangeHooks() {
  const tables = [
    ...
    'recipeGroups', 'recipeCategories', 'recipes', 'recipeIngredients', 'recipeProductLinks',     'recipeProductCategoryLinks', 'recipeProductGroupLinks', 'productRecipeComponents', 'productPortionComponents', 'productFlowLinks', 'supplierCategories', 'suppliers', 'rawMaterials', 'weeklyProductionPlans', 'weeklyProductionPlanItems',
...
export async function getBackupStatus() {
  ...
  let liveSync = null;
  try {
    const { getLiveSyncStatus } = await import('./supabase-sync.js?v=480');
