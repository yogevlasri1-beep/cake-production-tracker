import { initDB } from './db.js?v=483';
import { renderHome, homeMeta } from './screens/home.js?v=483';
import { renderProducts, productsMeta } from './screens/products.js?v=483';
import { renderManager, managerMeta } from './screens/manager.js?v=483';
import { renderProcess, processMeta } from './screens/process.js?v=483';
import { renderReports, reportsMeta } from './screens/reports.js?v=483';
import { renderBackup, backupMeta } from './screens/backup.js?v=483';
import { renderFinance, financeMeta } from './screens/finance.js?v=483';
import { renderRecipes, recipesMeta, initRecipesSubNav } from './screens/recipes.js?v=483';
import { renderSuppliers, suppliersMeta, initSuppliersSubNav } from './screens/suppliers.js?v=483';
import { renderHaccp, haccpMeta } from './screens/haccp.js?v=483';
import { renderAccounts, accountsMeta } from './screens/accounts.js?v=483';
import { renderLots, lotsMeta } from './screens/lots.js?v=483';
import { renderInventory, inventoryMeta } from './screens/inventory.js?v=483';
import { renderProductCatalog, productCatalogMeta } from './screens/product-catalog.js?v=483';
import { getSavedWorkspace, saveWorkspace, WORKSPACES, MANAGER_TAB_KEY } from './workspaces.js?v=483';
import { initIOSInstallPrompt } from './ios-install.js?v=483';
import { initNetworkCheck } from './network.js?v=483';
import { registerServiceWorker } from './sw-register.js?v=483';
import { APP_VERSION } from './version.js?v=483';
import { showToast } from './utils.js?v=483';
import { getCurrentUserRole, getCurrentWorkspaceAccess, signOut } from './auth.js?v=483';
import { allowedWorkspaces, canAccessWorkspace, PERMISSION_DENIED_MESSAGE } from './permissions.js?v=483';
import './modal.js?v=483';
...
async function boot() {
  const { getValidSession } = await import('./auth.js?v=483');
  const session = await getValidSession();
  if (!session) {
    const { renderLoginGate } = await import('./screens/login.js?v=483');
  ...
  try {
    ...
    if (versionEl) {
      ...
      versionEl.addEventListener('click', async () => {
        const { forceAppUpdate } = await import('./sw-register.js?v=483');
        showToast('מעדכן...');
        await forceAppUpdate();
      });
      import('./sw-register.js?v=483').then(async ({ detectRemoteVersion }) => {
    ...
    } = await import('./supabase-sync.js?v=483');
    ...
    const { initAutoBackupSystem, promptRestoreIfNeeded } = await import('./backup-service.js?v=483');
