import { initDB } from './db.js?v=484';
import { renderHome, homeMeta } from './screens/home.js?v=484';
import { renderProducts, productsMeta } from './screens/products.js?v=484';
import { renderManager, managerMeta } from './screens/manager.js?v=484';
import { renderProcess, processMeta } from './screens/process.js?v=484';
import { renderReports, reportsMeta } from './screens/reports.js?v=484';
import { renderBackup, backupMeta } from './screens/backup.js?v=484';
import { renderFinance, financeMeta } from './screens/finance.js?v=484';
import { renderRecipes, recipesMeta, initRecipesSubNav } from './screens/recipes.js?v=484';
import { renderSuppliers, suppliersMeta, initSuppliersSubNav } from './screens/suppliers.js?v=484';
import { renderHaccp, haccpMeta } from './screens/haccp.js?v=484';
import { renderAccounts, accountsMeta } from './screens/accounts.js?v=484';
import { renderLots, lotsMeta } from './screens/lots.js?v=484';
import { renderInventory, inventoryMeta } from './screens/inventory.js?v=484';
import { renderProductCatalog, productCatalogMeta } from './screens/product-catalog.js?v=484';
import { getSavedWorkspace, saveWorkspace, WORKSPACES, MANAGER_TAB_KEY } from './workspaces.js?v=484';
import { initIOSInstallPrompt } from './ios-install.js?v=484';
import { initNetworkCheck } from './network.js?v=484';
import { registerServiceWorker } from './sw-register.js?v=484';
import { APP_VERSION } from './version.js?v=484';
import { showToast } from './utils.js?v=484';
import { getCurrentUserRole, getCurrentWorkspaceAccess, signOut } from './auth.js?v=484';
import { allowedWorkspaces, canAccessWorkspace, PERMISSION_DENIED_MESSAGE } from './permissions.js?v=484';
import './modal.js?v=484';
...
async function boot() {
  const { getValidSession } = await import('./auth.js?v=484');
  const session = await getValidSession();
  if (!session) {
    const { renderLoginGate } = await import('./screens/login.js?v=484');
  ...
  try {
    ...
    if (versionEl) {
      ...
      versionEl.addEventListener('click', async () => {
        const { forceAppUpdate } = await import('./sw-register.js?v=484');
        showToast('מעדכן...');
        await forceAppUpdate();
      });
      import('./sw-register.js?v=484').then(async ({ detectRemoteVersion }) => {
    ...
    } = await import('./supabase-sync.js?v=484');
    ...
    const { initAutoBackupSystem, promptRestoreIfNeeded } = await import('./backup-service.js?v=484');
