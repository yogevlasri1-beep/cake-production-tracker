import { initDB } from './db.js?v=443';
import { renderHome, homeMeta } from './screens/home.js?v=443';
import { renderProducts, productsMeta } from './screens/products.js?v=443';
import { renderManager, managerMeta } from './screens/manager.js?v=443';
import { renderProcess, processMeta } from './screens/process.js?v=443';
import { renderReports, reportsMeta } from './screens/reports.js?v=443';
import { renderBackup, backupMeta } from './screens/backup.js?v=443';
import { renderRecipes, recipesMeta, initRecipesSubNav } from './screens/recipes.js?v=443';
import { renderSuppliers, suppliersMeta, initSuppliersSubNav } from './screens/suppliers.js?v=443';
import { renderHaccp, haccpMeta } from './screens/haccp.js?v=443';
import { renderAccounts, accountsMeta } from './screens/accounts.js?v=443';
import { renderLots, lotsMeta } from './screens/lots.js?v=443';
import { renderInventory, inventoryMeta } from './screens/inventory.js?v=443';
import { renderProductCatalog, productCatalogMeta } from './screens/product-catalog.js?v=443';
import { getSavedWorkspace, saveWorkspace, WORKSPACES, MANAGER_TAB_KEY } from './workspaces.js?v=443';
import { initIOSInstallPrompt } from './ios-install.js?v=443';
import { initNetworkCheck } from './network.js?v=443';
import { registerServiceWorker } from './sw-register.js?v=443';
import { APP_VERSION } from './version.js?v=443';
import { showToast } from './utils.js?v=443';
import { getCurrentUserRole, getCurrentWorkspaceAccess } from './auth.js?v=443';
import { allowedWorkspaces, canAccessWorkspace, PERMISSION_DENIED_MESSAGE } from './permissions.js?v=443';
import './modal.js?v=443';
...
async function boot() {
  const { getValidSession } = await import('./auth.js?v=443');
  const session = await getValidSession();
  if (!session) {
    const { renderLoginGate } = await import('./screens/login.js?v=443');
  ...
  try {
    ...
    if (versionEl) {
      ...
      versionEl.addEventListener('click', async () => {
        const { forceAppUpdate } = await import('./sw-register.js?v=443');
        showToast('מעדכן...');
        await forceAppUpdate();
      });
      import('./sw-register.js?v=443').then(async ({ detectRemoteVersion }) => {
    ...
    } = await import('./supabase-sync.js?v=443');
    ...
    const { initAutoBackupSystem, promptRestoreIfNeeded } = await import('./backup-service.js?v=443');
