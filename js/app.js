import { initDB } from './db.js?v=459';
import { renderHome, homeMeta } from './screens/home.js?v=459';
import { renderProducts, productsMeta } from './screens/products.js?v=459';
import { renderManager, managerMeta } from './screens/manager.js?v=459';
import { renderProcess, processMeta } from './screens/process.js?v=459';
import { renderReports, reportsMeta } from './screens/reports.js?v=459';
import { renderBackup, backupMeta } from './screens/backup.js?v=459';
import { renderRecipes, recipesMeta, initRecipesSubNav } from './screens/recipes.js?v=459';
import { renderSuppliers, suppliersMeta, initSuppliersSubNav } from './screens/suppliers.js?v=459';
import { renderHaccp, haccpMeta } from './screens/haccp.js?v=459';
import { renderAccounts, accountsMeta } from './screens/accounts.js?v=459';
import { renderLots, lotsMeta } from './screens/lots.js?v=459';
import { renderInventory, inventoryMeta } from './screens/inventory.js?v=459';
import { renderProductCatalog, productCatalogMeta } from './screens/product-catalog.js?v=459';
import { getSavedWorkspace, saveWorkspace, WORKSPACES, MANAGER_TAB_KEY } from './workspaces.js?v=459';
import { initIOSInstallPrompt } from './ios-install.js?v=459';
import { initNetworkCheck } from './network.js?v=459';
import { registerServiceWorker } from './sw-register.js?v=459';
import { APP_VERSION } from './version.js?v=459';
import { showToast } from './utils.js?v=459';
import { getCurrentUserRole, getCurrentWorkspaceAccess } from './auth.js?v=459';
import { allowedWorkspaces, canAccessWorkspace, PERMISSION_DENIED_MESSAGE } from './permissions.js?v=459';
import './modal.js?v=459';
...
async function boot() {
  const { getValidSession } = await import('./auth.js?v=459');
  const session = await getValidSession();
  if (!session) {
    const { renderLoginGate } = await import('./screens/login.js?v=459');
  ...
  try {
    ...
    if (versionEl) {
      ...
      versionEl.addEventListener('click', async () => {
        const { forceAppUpdate } = await import('./sw-register.js?v=459');
        showToast('מעדכן...');
        await forceAppUpdate();
      });
      import('./sw-register.js?v=459').then(async ({ detectRemoteVersion }) => {
    ...
    } = await import('./supabase-sync.js?v=459');
    ...
    const { initAutoBackupSystem, promptRestoreIfNeeded } = await import('./backup-service.js?v=459');
