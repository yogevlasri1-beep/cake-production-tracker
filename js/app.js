import { initDB } from './db.js?v=480';
import { renderHome, homeMeta } from './screens/home.js?v=480';
import { renderProducts, productsMeta } from './screens/products.js?v=480';
import { renderManager, managerMeta } from './screens/manager.js?v=480';
import { renderProcess, processMeta } from './screens/process.js?v=480';
import { renderReports, reportsMeta } from './screens/reports.js?v=480';
import { renderBackup, backupMeta } from './screens/backup.js?v=480';
import { renderFinance, financeMeta } from './screens/finance.js?v=480';
import { renderRecipes, recipesMeta, initRecipesSubNav } from './screens/recipes.js?v=480';
import { renderSuppliers, suppliersMeta, initSuppliersSubNav } from './screens/suppliers.js?v=480';
import { renderHaccp, haccpMeta } from './screens/haccp.js?v=480';
import { renderAccounts, accountsMeta } from './screens/accounts.js?v=480';
import { renderLots, lotsMeta } from './screens/lots.js?v=480';
import { renderInventory, inventoryMeta } from './screens/inventory.js?v=480';
import { renderProductCatalog, productCatalogMeta } from './screens/product-catalog.js?v=480';
import { getSavedWorkspace, saveWorkspace, WORKSPACES, MANAGER_TAB_KEY } from './workspaces.js?v=480';
import { initIOSInstallPrompt } from './ios-install.js?v=480';
import { initNetworkCheck } from './network.js?v=480';
...
async function boot() {
  const { getValidSession } = await import('./auth.js?v=480');
  const session = await getValidSession();
  if (!session) {
    const { renderLoginGate } = await import('./screens/login.js?v=480');
  ...
  try {
    ...
    if (versionEl) {
      ...
      versionEl.addEventListener('click', async () => {
        const { forceAppUpdate } = await import('./sw-register.js?v=480');
        showToast('מעדכן...');
        await forceAppUpdate();
      });
      import('./sw-register.js?v=480').then(async ({ detectRemoteVersion }) => {
    ...
    } = await import('./supabase-sync.js?v=480');
    ...
    const { initAutoBackupSystem, promptRestoreIfNeeded } = await import('./backup-service.js?v=480');
