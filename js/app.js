import { initDB } from './db.js?v=421';
import { renderHome, homeMeta } from './screens/home.js?v=421';
import { renderProducts, productsMeta } from './screens/products.js?v=421';
import { renderManager, managerMeta } from './screens/manager.js?v=421';
import { renderProcess, processMeta } from './screens/process.js?v=421';
import { renderReports, reportsMeta } from './screens/reports.js?v=421';
import { renderBackup, backupMeta } from './screens/backup.js?v=421';
import { renderRecipes, recipesMeta, initRecipesSubNav } from './screens/recipes.js?v=421';
import { renderSuppliers, suppliersMeta, initSuppliersSubNav } from './screens/suppliers.js?v=421';
import { renderHaccp, haccpMeta } from './screens/haccp.js?v=421';
import { renderAccounts, accountsMeta } from './screens/accounts.js?v=421';
import { renderLots, lotsMeta } from './screens/lots.js?v=421';
import { renderInventory, inventoryMeta } from './screens/inventory.js?v=421';
import { getSavedWorkspace, saveWorkspace, WORKSPACES, MANAGER_TAB_KEY } from './workspaces.js?v=421';
import { initIOSInstallPrompt } from './ios-install.js?v=421';
import { initNetworkCheck } from './network.js?v=421';
import { registerServiceWorker } from './sw-register.js?v=421';
import { APP_VERSION } from './version.js?v=421';
import { showToast } from './utils.js?v=421';
import { getCurrentUserRole } from './auth.js?v=421';
import { allowedWorkspaces, canAccessWorkspace, PERMISSION_DENIED_MESSAGE } from './permissions.js?v=421';
import './modal.js?v=421';
...
async function boot() {
  const { getValidSession } = await import('./auth.js?v=421');
  const session = await getValidSession();
  if (!session) {
    const { renderLoginGate } = await import('./screens/login.js?v=421');
  ...
  try {
    ...
    if (versionEl) {
      ...
      versionEl.addEventListener('click', async () => {
        const { forceAppUpdate } = await import('./sw-register.js?v=421');
        showToast('מעדכן...');
        await forceAppUpdate();
      });
      import('./sw-register.js?v=421').then(async ({ detectRemoteVersion }) => {
    ...
    } = await import('./supabase-sync.js?v=421');
    ...
    const { initAutoBackupSystem, promptRestoreIfNeeded } = await import('./backup-service.js?v=421');
