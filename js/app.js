import { initDB } from './db.js?v=438';
import { renderHome, homeMeta } from './screens/home.js?v=438';
import { renderProducts, productsMeta } from './screens/products.js?v=438';
import { renderManager, managerMeta } from './screens/manager.js?v=438';
import { renderProcess, processMeta } from './screens/process.js?v=438';
import { renderReports, reportsMeta } from './screens/reports.js?v=438';
import { renderBackup, backupMeta } from './screens/backup.js?v=438';
import { renderRecipes, recipesMeta, initRecipesSubNav } from './screens/recipes.js?v=438';
import { renderSuppliers, suppliersMeta, initSuppliersSubNav } from './screens/suppliers.js?v=438';
import { renderHaccp, haccpMeta } from './screens/haccp.js?v=438';
import { renderAccounts, accountsMeta } from './screens/accounts.js?v=438';
import { renderLots, lotsMeta } from './screens/lots.js?v=438';
import { renderInventory, inventoryMeta } from './screens/inventory.js?v=438';
import { getSavedWorkspace, saveWorkspace, WORKSPACES, MANAGER_TAB_KEY } from './workspaces.js?v=438';
import { initIOSInstallPrompt } from './ios-install.js?v=438';
import { initNetworkCheck } from './network.js?v=438';
import { registerServiceWorker } from './sw-register.js?v=438';
import { APP_VERSION } from './version.js?v=438';
import { showToast } from './utils.js?v=438';
import { getCurrentUserRole, getCurrentWorkspaceAccess } from './auth.js?v=438';
import { allowedWorkspaces, canAccessWorkspace, PERMISSION_DENIED_MESSAGE } from './permissions.js?v=438';
import './modal.js?v=438';
...
async function boot() {
  const { getValidSession } = await import('./auth.js?v=438');
  const session = await getValidSession();
  if (!session) {
    const { renderLoginGate } = await import('./screens/login.js?v=438');
  ...
  try {
    ...
    if (versionEl) {
      ...
      versionEl.addEventListener('click', async () => {
        const { forceAppUpdate } = await import('./sw-register.js?v=438');
        showToast('מעדכן...');
        await forceAppUpdate();
      });
      import('./sw-register.js?v=438').then(async ({ detectRemoteVersion }) => {
    ...
    } = await import('./supabase-sync.js?v=438');
    ...
    const { initAutoBackupSystem, promptRestoreIfNeeded } = await import('./backup-service.js?v=438');
