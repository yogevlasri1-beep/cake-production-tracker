import { initDB } from './db.js?v=470';
import { renderHome, homeMeta } from './screens/home.js?v=470';
import { renderProducts, productsMeta } from './screens/products.js?v=470';
import { renderManager, managerMeta } from './screens/manager.js?v=470';
import { renderProcess, processMeta } from './screens/process.js?v=470';
import { renderReports, reportsMeta } from './screens/reports.js?v=470';
import { renderBackup, backupMeta } from './screens/backup.js?v=470';
import { renderRecipes, recipesMeta, initRecipesSubNav } from './screens/recipes.js?v=470';
import { renderSuppliers, suppliersMeta, initSuppliersSubNav } from './screens/suppliers.js?v=470';
import { renderHaccp, haccpMeta } from './screens/haccp.js?v=470';
import { renderAccounts, accountsMeta } from './screens/accounts.js?v=470';
import { renderLots, lotsMeta } from './screens/lots.js?v=470';
import { renderInventory, inventoryMeta } from './screens/inventory.js?v=470';
import { renderProductCatalog, productCatalogMeta } from './screens/product-catalog.js?v=470';
import { getSavedWorkspace, saveWorkspace, WORKSPACES, MANAGER_TAB_KEY } from './workspaces.js?v=470';
import { initIOSInstallPrompt } from './ios-install.js?v=470';
import { initNetworkCheck } from './network.js?v=470';
import { registerServiceWorker } from './sw-register.js?v=470';
import { APP_VERSION } from './version.js?v=470';
import { showToast } from './utils.js?v=470';
import { getCurrentUserRole, getCurrentWorkspaceAccess } from './auth.js?v=470';
import { allowedWorkspaces, canAccessWorkspace, PERMISSION_DENIED_MESSAGE } from './permissions.js?v=470';
import './modal.js?v=470';
...
async function boot() {
  const { getValidSession } = await import('./auth.js?v=470');
  const session = await getValidSession();
  if (!session) {
    const { renderLoginGate } = await import('./screens/login.js?v=470');
  ...
  try {
    ...
    if (versionEl) {
      ...
      versionEl.addEventListener('click', async () => {
        const { forceAppUpdate } = await import('./sw-register.js?v=470');
        showToast('מעדכן...');
        await forceAppUpdate();
      });
      import('./sw-register.js?v=470').then(async ({ detectRemoteVersion }) => {
    ...
    } = await import('./supabase-sync.js?v=470');
    ...
    const { initAutoBackupSystem, promptRestoreIfNeeded } = await import('./backup-service.js?v=470');
