import { initDB } from './db.js?v=100';
import { renderHome, homeMeta } from './screens/home.js?v=100';
import { renderRecord, recordMeta } from './screens/record.js?v=100';
import { renderProducts, productsMeta } from './screens/products.js?v=100';
import { renderManager, managerMeta } from './screens/manager.js?v=100';
import { renderProcess, processMeta } from './screens/process.js?v=100';
import { renderReports, reportsMeta } from './screens/reports.js?v=100';
import { renderBackup, backupMeta } from './screens/backup.js?v=100';
import { initIOSInstallPrompt } from './ios-install.js?v=100';
import { initNetworkCheck } from './network.js?v=100';
import { registerServiceWorker } from './sw-register.js?v=100';
import { APP_VERSION } from './version.js?v=100';
import './modal.js?v=100';
  ...
  try {
    ...
    if (versionEl) {
      ...
      versionEl.addEventListener('click', async () => {
        const { forceAppUpdate, detectRemoteVersion } = await import('./sw-register.js?v=100');
      ...
      import('./sw-register.js?v=100').then(async ({ detectRemoteVersion }) => {
        const remote = await detectRemoteVersion();
        if (remote && remote !== APP_VERSION) {
    ...
    const { initAutoBackupSystem, promptRestoreIfNeeded } = await import('./backup-service.js?v=100');
