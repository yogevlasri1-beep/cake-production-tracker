import { initDB } from './db.js?v=420';
import { renderHome, homeMeta } from './screens/home.js?v=420';
import { renderProducts, productsMeta } from './screens/products.js?v=420';
import { renderManager, managerMeta } from './screens/manager.js?v=420';
import { renderProcess, processMeta } from './screens/process.js?v=420';
import { renderReports, reportsMeta } from './screens/reports.js?v=420';
import { renderBackup, backupMeta } from './screens/backup.js?v=420';
import { renderRecipes, recipesMeta, initRecipesSubNav } from './screens/recipes.js?v=420';
import { renderSuppliers, suppliersMeta, initSuppliersSubNav } from './screens/suppliers.js?v=420';
import { renderHaccp, haccpMeta } from './screens/haccp.js?v=420';
import { renderAccounts, accountsMeta } from './screens/accounts.js?v=420';
import { renderLots, lotsMeta } from './screens/lots.js?v=420';
import { renderInventory, inventoryMeta } from './screens/inventory.js?v=420';
import { getSavedWorkspace, saveWorkspace, WORKSPACES, MANAGER_TAB_KEY } from './workspaces.js?v=420';
import { initIOSInstallPrompt } from './ios-install.js?v=420';
import { initNetworkCheck } from './network.js?v=420';
import { registerServiceWorker } from './sw-register.js?v=420';
import { APP_VERSION } from './version.js?v=420';
import { showToast } from './utils.js?v=420';
import { getCurrentUserRole } from './auth.js?v=420';
import { allowedWorkspaces, canAccessWorkspace, PERMISSION_DENIED_MESSAGE } from './permissions.js?v=420';
import './modal.js?v=420';
...
async function boot() {
  const { getValidSession } = await import('./auth.js?v=420');
  const session = await getValidSession();
  if (!session) {
    const { renderLoginGate } = await import('./screens/login.js?v=420');
  ...
  try {
    ...
    if (versionEl) {
      ...
      versionEl.addEventListener('click', async () => {
        const { forceAppUpdate } = await import('./sw-register.js?v=420');
        showToast('מעדכן...');
        await forceAppUpdate();
      });
      import('./sw-register.js?v=420').then(async ({ detectRemoteVersion }) => {
    ...
    } = await import('./supabase-sync.js?v=420');
    ...
    const { initAutoBackupSystem, promptRestoreIfNeeded } = await import('./backup-service.js?v=420');
