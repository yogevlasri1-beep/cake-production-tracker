import { escapeHtml, showToast, formatDateTime, weekStartISO, todayISO } from '../utils.js?v=483';
...
} from '../inventory-db.js?v=483';
import { getSupplierCategories, findRawMaterialsByBarcode } from '../kitchen-db.js?v=483';
import { getCurrentUserRole } from '../auth.js?v=483';
import { canAdjustInventory, PERMISSION_DENIED_MESSAGE } from '../permissions.js?v=483';
  ...
  document.getElementById('inv-save')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!hasSet && packagingBatchNumber && Number(deltaVal) > 0) {
        const { receiveInventoryLot } = await import('../inventory-db.js?v=483');
