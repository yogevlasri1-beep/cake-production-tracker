import { escapeHtml, showToast, formatDateTime, weekStartISO, todayISO } from '../utils.js?v=475';
...
} from '../inventory-db.js?v=475';
import { getSupplierCategories, findRawMaterialsByBarcode } from '../kitchen-db.js?v=475';
import { getCurrentUserRole } from '../auth.js?v=475';
import { canAdjustInventory, PERMISSION_DENIED_MESSAGE } from '../permissions.js?v=475';
  ...
  document.getElementById('inv-save')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!hasSet && packagingBatchNumber && Number(deltaVal) > 0) {
        const { receiveInventoryLot } = await import('../inventory-db.js?v=475');
