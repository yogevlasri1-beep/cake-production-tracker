import { escapeHtml, showToast, formatDateTime, weekStartISO, todayISO } from '../utils.js?v=477';
...
} from '../inventory-db.js?v=477';
import { getSupplierCategories, findRawMaterialsByBarcode } from '../kitchen-db.js?v=477';
import { getCurrentUserRole } from '../auth.js?v=477';
import { canAdjustInventory, PERMISSION_DENIED_MESSAGE } from '../permissions.js?v=477';
  ...
  document.getElementById('inv-save')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!hasSet && packagingBatchNumber && Number(deltaVal) > 0) {
        const { receiveInventoryLot } = await import('../inventory-db.js?v=477');
