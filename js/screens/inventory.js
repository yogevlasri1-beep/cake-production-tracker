import { escapeHtml, showToast, formatDateTime, weekStartISO, todayISO } from '../utils.js?v=474';
...
} from '../inventory-db.js?v=474';
import { getSupplierCategories, findRawMaterialsByBarcode } from '../kitchen-db.js?v=474';
import { getCurrentUserRole } from '../auth.js?v=474';
import { canAdjustInventory, PERMISSION_DENIED_MESSAGE } from '../permissions.js?v=474';
  ...
  document.getElementById('inv-save')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!hasSet && packagingBatchNumber && Number(deltaVal) > 0) {
        const { receiveInventoryLot } = await import('../inventory-db.js?v=474');
