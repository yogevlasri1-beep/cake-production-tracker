...
} from '../backup-service.js?v=436';
import { describeDownloadMethod } from '../download.js?v=436';
import { showToast, escapeHtml } from '../utils.js?v=436';
import { openModal, closeModal } from '../modal.js?v=436';
import { APP_VERSION } from '../version.js?v=436';
import { forceAppUpdate, checkForAppUpdate, detectRemoteVersion, isStandaloneApp } from '../sw-register.js?v=436';
import { getCurrentUserEmail, getCurrentUserRole, userRoleLabel, signOut } from '../auth.js?v=436';
import { canAccessBackupFull } from '../permissions.js?v=436';
...
export async function renderBackup(container, { navigate } = {}) {
  ...
  document.getElementById('live-sync-enabled')?.addEventListener('change', async (e) => {
    try {
      const { setLiveSyncEnabled } = await import('../supabase-sync.js?v=436');
  ...
  document.getElementById('live-sync-now')?.addEventListener('click', async () => {
    ...
    try {
      const { flushSyncQueue, pullAllCollections } = await import('../supabase-sync.js?v=436');
  ...
  document.getElementById('live-sync-reseed')?.addEventListener('click', async () => {
    ...
    try {
      const { seedLocalDataToSupabase, saveLiveSyncSettings } = await import('../supabase-sync.js?v=436');
  ...
  document.getElementById('live-sync-reset')?.addEventListener('click', async () => {
    ...
    try {
      const { resetLocalSyncState } = await import('../supabase-sync.js?v=436');
  ...
  document.getElementById('live-sync-dedupe')?.addEventListener('click', async () => {
    ...
    try {
      const { dedupeLocalSyncCollections, flushSyncQueue, pullAllCollections, saveLiveSyncSettings } = await import('../supabase-sync.js?v=436');
