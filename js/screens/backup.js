...
} from '../backup-service.js?v=383';
import { describeDownloadMethod } from '../download.js?v=383';
import { showToast, escapeHtml } from '../utils.js?v=383';
import { openModal, closeModal } from '../modal.js?v=383';
import { APP_VERSION } from '../version.js?v=383';
import { forceAppUpdate, checkForAppUpdate, detectRemoteVersion, isStandaloneApp } from '../sw-register.js?v=383';
...
export async function renderBackup(container, { navigate } = {}) {
  ...
  document.getElementById('live-sync-enabled')?.addEventListener('change', async (e) => {
    try {
      const { setLiveSyncEnabled } = await import('../supabase-sync.js?v=383');
  ...
  document.getElementById('live-sync-now')?.addEventListener('click', async () => {
    ...
    try {
      const { flushSyncQueue, pullAllCollections } = await import('../supabase-sync.js?v=383');
  ...
  document.getElementById('live-sync-reseed')?.addEventListener('click', async () => {
    ...
    try {
      const { seedLocalDataToSupabase, saveLiveSyncSettings } = await import('../supabase-sync.js?v=383');
  ...
  document.getElementById('live-sync-reset')?.addEventListener('click', async () => {
    ...
    try {
      const { resetLocalSyncState } = await import('../supabase-sync.js?v=383');
  ...
  document.getElementById('live-sync-dedupe')?.addEventListener('click', async () => {
    ...
    try {
      const { dedupeLocalSyncCollections, flushSyncQueue, pullAllCollections, saveLiveSyncSettings } = await import('../supabase-sync.js?v=383');
