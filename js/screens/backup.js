...
} from '../backup-service.js?v=368';
import { describeDownloadMethod } from '../download.js?v=368';
import { showToast, escapeHtml } from '../utils.js?v=368';
import { openModal, closeModal } from '../modal.js?v=368';
import { APP_VERSION } from '../version.js?v=368';
import { forceAppUpdate, checkForAppUpdate, detectRemoteVersion, isStandaloneApp } from '../sw-register.js?v=368';
...
export async function renderBackup(container, { navigate } = {}) {
  ...
  document.getElementById('live-sync-enabled')?.addEventListener('change', async (e) => {
    try {
      const { setLiveSyncEnabled } = await import('../supabase-sync.js?v=368');
  ...
  document.getElementById('live-sync-now')?.addEventListener('click', async () => {
    ...
    try {
      const { flushSyncQueue, pullAllCollections } = await import('../supabase-sync.js?v=368');
  ...
  document.getElementById('live-sync-reseed')?.addEventListener('click', async () => {
    ...
    try {
      const { seedLocalDataToSupabase, saveLiveSyncSettings } = await import('../supabase-sync.js?v=368');
  ...
  document.getElementById('live-sync-dedupe')?.addEventListener('click', async () => {
    ...
    try {
      const { dedupeLocalSyncCollections, flushSyncQueue, pullAllCollections, saveLiveSyncSettings } = await import('../supabase-sync.js?v=368');
