...
function wirePortionIngredientsButtons(root, { onSaved } = {}) {
  import('../portion-ingredients.js?v=403').then(({ bindPortionIngredientsButtons }) => {
    bindPortionIngredientsButtons(root, { onSaved });
  }).catch((err) => {
    console.warn('portion-ingredients load failed', err);
  });
}
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, formatStopwatch, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=403';
import { openModal, closeModal } from '../modal.js?v=403';
import { requestAutoBackupNow } from '../backup-service.js?v=403';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=403';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=403';
import { materialMatchesSearch } from '../kitchen-db.js?v=403';
...
async function openRunPortionsWeightModal(run) {
  ...
  try {
    const { getRecipe } = await import('../kitchen-db.js?v=403');
    const { db } = await import('../db.js?v=403');
    const blocks = [];
...
async function renderRunView(container, runId, ctx) {
  ...
  try {
    const kitchen = await import('../kitchen-db.js?v=403');
