...
function wirePortionIngredientsButtons(root, { onSaved } = {}) {
  import('../portion-ingredients.js?v=447').then(({ bindPortionIngredientsButtons }) => {
...
async function offerInventoryIssueForPortion(run, entry, {
  ...
  try {
    ...
    } = await import('../inventory-db.js?v=447');
...
async function reverseInventoryIssueForPortion(run, entry) {
  ...
  try {
    const { reverseStockIssueLines } = await import('../inventory-db.js?v=447');
...
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, formatStopwatch, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=447';
import { openModal, closeModal } from '../modal.js?v=447';
import { requestAutoBackupNow } from '../backup-service.js?v=447';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=447';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=447';
import { materialMatchesSearch } from '../kitchen-db.js?v=447';
import { getCurrentUserRole } from '../auth.js?v=447';
import { canManageFlows, PERMISSION_DENIED_MESSAGE } from '../permissions.js?v=447';
...
async function openRunPortionsWeightModal(run) {
  ...
  try {
    const { getRecipe } = await import('../kitchen-db.js?v=447');
    const { db } = await import('../db.js?v=447');
    const blocks = [];
...
async function renderRunView(container, runId, ctx) {
  ...
  try {
    const kitchen = await import('../kitchen-db.js?v=447');
