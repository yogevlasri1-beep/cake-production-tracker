...
function wirePortionIngredientsButtons(root, { onSaved } = {}) {
  import('../portion-ingredients.js?v=301').then(({ bindPortionIngredientsButtons }) => {
    bindPortionIngredientsButtons(root, { onSaved });
  }).catch((err) => {
    console.warn('portion-ingredients load failed', err);
  });
}
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, formatStopwatch, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=301';
import { openModal, closeModal } from '../modal.js?v=301';
import { requestAutoBackupNow } from '../backup-service.js?v=301';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=301';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=301';
...
async function openRunPortionsWeightModal(run) {
  ...
  try {
    const { getRecipe } = await import('../kitchen-db.js?v=301');
    const { db } = await import('../db.js?v=301');
    const blocks = [];
