...
function wirePortionIngredientsButtons(root, { onSaved } = {}) {
  import('../portion-ingredients.js?v=289').then(({ bindPortionIngredientsButtons }) => {
    bindPortionIngredientsButtons(root, { onSaved });
  }).catch((err) => {
    console.warn('portion-ingredients load failed', err);
  });
}
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=289';
import { openModal, closeModal } from '../modal.js?v=289';
import { requestAutoBackupNow } from '../backup-service.js?v=289';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=289';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=289';
