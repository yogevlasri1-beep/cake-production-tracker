...
function wirePortionIngredientsButtons(root, { onSaved } = {}) {
  import('../portion-ingredients.js?v=311').then(({ bindPortionIngredientsButtons }) => {
    bindPortionIngredientsButtons(root, { onSaved });
  }).catch((err) => {
    console.warn('portion-ingredients load failed', err);
  });
}
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, formatStopwatch, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=311';
import { openModal, closeModal } from '../modal.js?v=311';
import { requestAutoBackupNow } from '../backup-service.js?v=311';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=311';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=311';
...
async function openRunPortionsWeightModal(run) {
  ...
  try {
    const { getRecipe } = await import('../kitchen-db.js?v=311');
    const { db } = await import('../db.js?v=311');
    const blocks = [];
