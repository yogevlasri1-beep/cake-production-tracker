...
function wirePortionIngredientsButtons(root, { onSaved } = {}) {
  import('../portion-ingredients.js?v=364').then(({ bindPortionIngredientsButtons }) => {
    bindPortionIngredientsButtons(root, { onSaved });
  }).catch((err) => {
    console.warn('portion-ingredients load failed', err);
  });
}
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, formatStopwatch, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=364';
import { openModal, closeModal } from '../modal.js?v=364';
import { requestAutoBackupNow } from '../backup-service.js?v=364';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=364';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=364';
import { materialMatchesSearch } from '../kitchen-db.js?v=364';
...
async function openRunPortionsWeightModal(run) {
  ...
  try {
    const { getRecipe } = await import('../kitchen-db.js?v=364');
    const { db } = await import('../db.js?v=364');
    const blocks = [];
...
async function renderRunView(container, runId, ctx) {
  ...
  try {
    const kitchen = await import('../kitchen-db.js?v=364');
