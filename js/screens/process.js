...
function wirePortionIngredientsButtons(root, { onSaved } = {}) {
  import('../portion-ingredients.js?v=462').then(({ bindPortionIngredientsButtons }) => {
...
async function offerInventoryIssueForPortion(run, entry, {
  ...
  try {
    ...
    } = await import('../inventory-db.js?v=462');
...
async function reverseInventoryIssueForPortion(run, entry) {
  ...
  try {
    const { reverseStockIssueLines } = await import('../inventory-db.js?v=462');
...
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, formatStopwatch, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=462';
import { openModal, closeModal } from '../modal.js?v=462';
import { requestAutoBackupNow } from '../backup-service.js?v=462';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=462';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=462';
import { materialMatchesSearch } from '../kitchen-db.js?v=462';
import { getCurrentUserRole } from '../auth.js?v=462';
import { canManageFlows, PERMISSION_DENIED_MESSAGE } from '../permissions.js?v=462';
...
async function openRunPortionsWeightModal(run) {
  ...
  try {
    const { getRecipe } = await import('../kitchen-db.js?v=462');
    const { db } = await import('../db.js?v=462');
    const blocks = [];
...
async function renderRunView(container, runId, ctx) {
  ...
  try {
    const kitchen = await import('../kitchen-db.js?v=462');
