...
function wirePortionIngredientsButtons(root, { onSaved } = {}) {
  import('../portion-ingredients.js?v=466').then(({ bindPortionIngredientsButtons }) => {
...
async function offerInventoryIssueForPortion(run, entry, {
  ...
  try {
    ...
    } = await import('../inventory-db.js?v=466');
...
async function reverseInventoryIssueForPortion(run, entry) {
  ...
  try {
    const { reverseStockIssueLines } = await import('../inventory-db.js?v=466');
...
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, formatStopwatch, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=466';
import { openModal, closeModal } from '../modal.js?v=466';
import { requestAutoBackupNow } from '../backup-service.js?v=466';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=466';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=466';
import { materialMatchesSearch } from '../kitchen-db.js?v=466';
import { getCurrentUserRole } from '../auth.js?v=466';
import { canManageFlows, PERMISSION_DENIED_MESSAGE } from '../permissions.js?v=466';
...
async function openRunPortionsWeightModal(run) {
  ...
  try {
    const { getRecipe } = await import('../kitchen-db.js?v=466');
    const { db } = await import('../db.js?v=466');
    const blocks = [];
...
async function renderRunView(container, runId, ctx) {
  ...
  try {
    const kitchen = await import('../kitchen-db.js?v=466');
