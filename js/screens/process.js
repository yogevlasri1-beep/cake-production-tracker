...
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, formatStopwatch, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=371';
import { openModal, closeModal } from '../modal.js?v=371';
import { requestAutoBackupNow } from '../backup-service.js?v=371';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=371';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=371';
import { materialMatchesSearch } from '../kitchen-db.js?v=371';
...
async function openRunPortionsWeightModal(run) {
  ...
  try {
    const { getRecipe } = await import('../kitchen-db.js?v=371');
    const { db } = await import('../db.js?v=371');
    const blocks = [];
...
async function renderRunView(container, runId, ctx) {
  ...
  try {
    const kitchen = await import('../kitchen-db.js?v=371');
