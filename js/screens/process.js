...
function wirePortionIngredientsButtons(root, { onSaved } = {}) {
  import('../portion-ingredients.js?v=431').then(({ bindPortionIngredientsButtons }) => {
...
async function offerInventoryIssueForPortion(run, entry, {
  ...
  try {
    ...
    } = await import('../inventory-db.js?v=431');
...
async function reverseInventoryIssueForPortion(run, entry) {
  ...
  try {
    const { reverseStockIssueLines } = await import('../inventory-db.js?v=431');
...
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, formatStopwatch, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=431';
import { openModal, closeModal } from '../modal.js?v=431';
import { requestAutoBackupNow } from '../backup-service.js?v=431';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=431';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=431';
import { materialMatchesSearch } from '../kitchen-db.js?v=431';
import { getCurrentUserRole } from '../auth.js?v=431';
import { canManageFlows, PERMISSION_DENIED_MESSAGE } from '../permissions.js?v=431';
...
async function openRunPortionsWeightModal(run) {
  ...
  try {
    const { getRecipe } = await import('../kitchen-db.js?v=431');
    const { db } = await import('../db.js?v=431');
    const blocks = [];
...
async function renderRunView(container, runId, ctx) {
  ...
  try {
    const kitchen = await import('../kitchen-db.js?v=431');
