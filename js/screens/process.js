...
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, formatStopwatch, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=370';
import { openModal, closeModal } from '../modal.js?v=370';
import { requestAutoBackupNow } from '../backup-service.js?v=370';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=370';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=370';
import { materialMatchesSearch } from '../kitchen-db.js?v=370';
...
async function openRunPortionsWeightModal(run) {
  ...
  try {
    const { getRecipe } = await import('../kitchen-db.js?v=370');
    const { db } = await import('../db.js?v=370');
    const blocks = [];
...
async function renderRunView(container, runId, ctx) {
  ...
  try {
    const kitchen = await import('../kitchen-db.js?v=370');
