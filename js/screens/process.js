...
function wirePortionIngredientsButtons(root, { onSaved } = {}) {
  import('../portion-ingredients.js?v=378').then(({ bindPortionIngredientsButtons }) => {
    bindPortionIngredientsButtons(root, { onSaved });
  }).catch((err) => {
    console.warn('portion-ingredients load failed', err);
  });
}
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, formatStopwatch, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=378';
import { openModal, closeModal } from '../modal.js?v=378';
import { requestAutoBackupNow } from '../backup-service.js?v=378';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=378';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=378';
import { materialMatchesSearch } from '../kitchen-db.js?v=378';
...
async function openRunPortionsWeightModal(run) {
  ...
  try {
    const { getRecipe } = await import('../kitchen-db.js?v=378');
    const { db } = await import('../db.js?v=378');
    const blocks = [];
...
async function renderRunView(container, runId, ctx) {
  ...
  try {
    const kitchen = await import('../kitchen-db.js?v=378');
