...
function wirePortionIngredientsButtons(root, { onSaved } = {}) {
  import('../portion-ingredients.js?v=330').then(({ bindPortionIngredientsButtons }) => {
    bindPortionIngredientsButtons(root, { onSaved });
  }).catch((err) => {
    console.warn('portion-ingredients load failed', err);
  });
}
import { todayISO, formatDate, showToast, escapeHtml, formatPortionCount, formatPortionWeightKg, formatProductQuantity, productRecordUsesKg, formatDuration, formatStopwatch, runDurationMs, stepDurationMs, getStepTimerElapsedMs, isoToDateInput, isoToTimeInput, formatDateTime, formatDecimal } from '../utils.js?v=330';
import { openModal, closeModal } from '../modal.js?v=330';
import { requestAutoBackupNow } from '../backup-service.js?v=330';
import { renderSheetsStatusHTML, bindSheetsStatusEvents } from '../sheets-flow.js?v=330';
import { bindFlowChecklistDragLists } from '../product-drag.js?v=330';
import { materialMatchesSearch } from '../kitchen-db.js?v=330';
...
async function openRunPortionsWeightModal(run) {
  ...
  try {
    const { getRecipe } = await import('../kitchen-db.js?v=330');
    const { db } = await import('../db.js?v=330');
    const blocks = [];
...
async function renderRunView(container, runId, ctx) {
  ...
  try {
    const kitchen = await import('../kitchen-db.js?v=330');
