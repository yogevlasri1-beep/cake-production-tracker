...
import { formatMoney, showToast, escapeHtml, productUnitLabel, productPriceUnitLabel, formatDecimal } from '../utils.js?v=445';
import { openModal, closeModal } from '../modal.js?v=445';
import { CATEGORY_COLOR_HEX, defaultColorForIndex } from '../chart.js?v=445';
import { bindProductDragLists, bindCategoryDragList, bindCategoryGroupDragList } from '../product-drag.js?v=445';
...
export async function renderProducts(container) {
  ...
  document.getElementById('open-backup-screen')?.addEventListener('click', async () => {
    const { navigate } = await import('../app.js?v=445');
    navigate('backup');
  });
...
function bindProductDetailModalEvents(container, productId, refreshModal) {
  ...
  async function openLinkedRecipe(recipeId) {
    ...
    if (!id) return;
    try {
      const { requestOpenRecipe } = await import('./recipes.js?v=445');
  ...
  document.getElementById('product-share-baking')?.addEventListener('click', async () => {
    try {
      const { shareBakingForProduct } = await import('./recipes.js?v=445');
  ...
  document.getElementById('product-allergens-recompute')?.addEventListener('click', async () => {
    try {
      const { computeProductAllergensFromComposition } = await import('../kitchen-db.js?v=445');
