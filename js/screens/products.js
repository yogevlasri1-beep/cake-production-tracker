...
import { formatMoney, showToast, escapeHtml, productUnitLabel, productPriceUnitLabel, formatDecimal } from '../utils.js?v=478';
import { openModal, closeModal } from '../modal.js?v=478';
import { CATEGORY_COLOR_HEX, defaultColorForIndex } from '../chart.js?v=478';
import { bindProductDragLists, bindCategoryDragList, bindCategoryGroupDragList } from '../product-drag.js?v=478';
...
function bindProductDetailModalEvents(container, productId, refreshModal) {
  ...
  async function openLinkedRecipe(recipeId) {
    ...
    if (!id) return;
    try {
      const { requestOpenRecipe } = await import('./recipes.js?v=478');
  ...
  document.getElementById('product-share-baking')?.addEventListener('click', async () => {
    try {
      const { shareBakingForProduct } = await import('./recipes.js?v=478');
  ...
  document.getElementById('product-allergens-recompute')?.addEventListener('click', async () => {
    try {
      const { computeProductAllergensFromComposition } = await import('../kitchen-db.js?v=478');
