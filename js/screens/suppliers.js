...
} from '../kitchen-db.js?v=475';
import { getProducts, getCategories } from '../db.js?v=475';
import {
...
} from '../supplier-import.js?v=475';
import { escapeHtml, showToast, formatMoney, weekStartISO, formatDate, todayISO } from '../utils.js?v=475';
import { openModal, closeModal } from '../modal.js?v=475';
import { requestAutoBackupNow } from '../backup-service.js?v=475';
import { bindSupplierDragList, bindMaterialDragList } from '../product-drag.js?v=475';
import { openBarcodeScanner } from '../barcode-scan.js?v=475';
import { getLiveSyncSettings, dedupeSupplierWorkspaceLight } from '../supabase-sync.js?v=475';
import {
...
async function renderShortagesTab(body, container) {
  ...
  body.querySelectorAll('.shortage-receive-btn').forEach((btn) => {
    btn.addEventListener('click', async () => {
      const { renderLotPickerFieldHTML, bindLotPickerFields } = await import('../lot-picker.js?v=475');
      openModal({
    ...
    inputHtml: '<input type="text" id="receive-lot-number" placeholder="מספר על האריזה">',
      ...
      document.getElementById('receive-lot-save')?.addEventListener('click', async () => {
        try {
          const { receiveShortageToInventory } = await import('../inventory-db.js?v=475');
  ...
  document.getElementById('receive-open-shortages')?.addEventListener('click', async () => {
    ...
    try {
      const { receiveOpenShortagesToInventory } = await import('../inventory-db.js?v=475');
