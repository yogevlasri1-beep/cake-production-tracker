...
import { getProducts, getCategories } from '../db.js?v=433';
import {
...
} from '../supplier-import.js?v=433';
import { escapeHtml, showToast, formatMoney, weekStartISO, formatDate, todayISO } from '../utils.js?v=433';
import { openModal, closeModal } from '../modal.js?v=433';
import { requestAutoBackupNow } from '../backup-service.js?v=433';
import { bindSupplierDragList, bindMaterialDragList } from '../product-drag.js?v=433';
...
async function renderShortagesTab(body, container) {
  ...
  body.querySelectorAll('.shortage-receive-btn').forEach((btn) => {
    btn.addEventListener('click', async () => {
      try {
        const { receiveShortageToInventory } = await import('../inventory-db.js?v=433');
  ...
  document.getElementById('receive-open-shortages')?.addEventListener('click', async () => {
    ...
    try {
      const { receiveOpenShortagesToInventory } = await import('../inventory-db.js?v=433');
