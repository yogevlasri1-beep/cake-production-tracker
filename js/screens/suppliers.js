...
} from '../kitchen-db.js?v=468';
import { getProducts, getCategories } from '../db.js?v=468';
import {
...
} from '../supplier-import.js?v=468';
import { escapeHtml, showToast, formatMoney, weekStartISO, formatDate, todayISO } from '../utils.js?v=468';
import { openModal, closeModal } from '../modal.js?v=468';
import { requestAutoBackupNow } from '../backup-service.js?v=468';
import { bindSupplierDragList, bindMaterialDragList } from '../product-drag.js?v=468';
import { openBarcodeScanner } from '../barcode-scan.js?v=468';
import { getLiveSyncSettings, dedupeSupplierWorkspaceLight } from '../supabase-sync.js?v=468';
import {
...
async function renderShortagesTab(body, container) {
  ...
  body.querySelectorAll('.shortage-receive-btn').forEach((btn) => {
    ...
    inputHtml: '<input type="text" id="receive-lot-number" placeholder="מספר על האריזה">',
      ...
      document.getElementById('receive-lot-save')?.addEventListener('click', async () => {
        try {
          const { receiveShortageToInventory } = await import('../inventory-db.js?v=468');
  ...
  document.getElementById('receive-open-shortages')?.addEventListener('click', async () => {
    ...
    try {
      const { receiveOpenShortagesToInventory } = await import('../inventory-db.js?v=468');
