...
} from '../kitchen-db.js?v=480';
import { getProducts, getCategories } from '../db.js?v=480';
import {
...
} from '../supplier-import.js?v=480';
import { escapeHtml, showToast, formatMoney, weekStartISO, formatDate, todayISO } from '../utils.js?v=480';
import { openModal, closeModal } from '../modal.js?v=480';
import { requestAutoBackupNow } from '../backup-service.js?v=480';
import { bindSupplierDragList, bindMaterialDragList } from '../product-drag.js?v=480';
import { openBarcodeScanner } from '../barcode-scan.js?v=480';
import { getLiveSyncSettings, dedupeSupplierWorkspaceLight } from '../supabase-sync.js?v=480';
import {
...
export async function renderSuppliers(container) {
  ...
  // ניקוי ברקע — לא חוסם את הצגת המסך (היה מאט כל מעבר טאב/שמירה)
  dedupeSupplierWorkspaceLight()
    .then((result) => {
      if (result?.skipped || !(result?.removed || result?.repaired)) return;
      // רק אם באמת תוקן משהו — רענון חד-פעמי בלי לולאה
      if (container.dataset.supplierMaintRefresh === '1') return;
      container.dataset.supplierMaintRefresh = '1';
      renderSuppliers(container).finally(() => {
        delete container.dataset.supplierMaintRefresh;
      });
    })
    .catch(() => {});
...
function supplierCategoryIcon(cat) {
  if (cat?.isCleaning) return '🧹 ';
  if (cat?.isPackaging) return '📦 ';
  return '';
}

/** שם היחידות בקטגוריה: חומרים / אריזות / חומרי ניקיון */
function supplierCategoryItemsLabel(cat) {
...
function renderSupplierCategoryChipLabel(cat) {
  return `${supplierCategoryIcon(cat)}${escapeHtml(cat.name)}`;
}
...
function renderMaterialPricingDetailsHTML(mat, { simple = false } = {}) {
  if (simple) {
    return `
    <div class="material-pricing-grid">
      <div class="material-pricing-cell">
        <span class="material-pricing-label">מחיר ליחידה</span>
        <strong>${(mat.unitPrice || 0) > 0 ? formatMoney(mat.unitPrice) : '—'}</strong>
      </div>
    </div>`;
  }
...
/**
 * איחוד נבחרים — אותה רשימה כמו מחסן (כרטיס אחד לשם חומר), לא כל שורת ספק בנפרד.
 * אחרי איחוד החלון נשאר פתוח באותו מקום להמשך איחודים; סגירה מרעננת את המחסן.
 */
async function openMergeSelectedMaterialsModal(container) {
  const catId = container.dataset.matCat ? Number(container.dataset.matCat) : null;
  ...
  if (catalog.length < 2) {
    showToast(catId
      ? 'בקטגוריה הזו יש פחות מ־2 חומרים לאיחוד — נסה «הכל» או קטגוריה אחרת'
      : 'צריך לפחות 2 חומרי גלם לאיחוד');
    return;
  }
  const catName = catId
    ? (categories.find((c) => c.id === catId)?.name || '')
    : '';
  let catalogByKey = new Map(catalog.map((item) => [item.key, item]));
  let sorted = [...catalog];
  ...
  function catalogItemMeta(item) {
    const best = pickBestOffer(item.offers);
    const parts = [];
    parts.push(item.supplierCount ? `${item.supplierCount} ספקים` : 'ללא ספק');
    const price = formatMaterialPriceMeta(best);
    if (price && price !== '—') parts.push(price);
    if (item.recipeDefaultId) parts.push('★ ברירת מחדל');
    return parts.join(' · ');
  }

  function keepIdForItem(item) {
    if (!item) return null;
    return item.recipeDefaultId || item.primaryId;
  }

  function renderOptionRow(item) {
    const checked = selectedKeys.has(item.key);
    return `
      <div class="merge-product-option manual-merge-option manual-mat-merge-option" data-catalog-key="${escapeHtml(item.key)}">
        <label class="manual-mat-merge-select">
          <input type="checkbox" class="manual-mat-merge-check" value="${escapeHtml(item.key)}"${checked ? ' checked' : ''}>
          <span class="manual-mat-merge-option-body">
            <strong>${escapeHtml(item.name)}</strong>
            <span class="merge-product-meta">${escapeHtml(catalogItemMeta(item))}</span>
          </span>
        </label>
        <label class="manual-mat-merge-keep" title="שם החומר שיישאר במחסן">
          <input type="radio" name="manual-mat-keep" class="manual-mat-keep-radio" value="${escapeHtml(item.key)}"${checked ? '' : ' disabled'}${keepKey === item.key ? ' checked' : ''}>
          <span>יעד</span>
        </label>
      </div>`;
  }

  function filterCatalogForMerge(search) {
    const q = String(search || '').trim().toLocaleLowerCase('he');
    if (!q) return sorted;
    return filterCatalogItems(sorted, q);
  }

  function renderOptions(search = '') {
    const matchesAll = filterCatalogForMerge(search);
    const selected = sorted.filter((item) => selectedKeys.has(item.key));
    const matches = matchesAll.filter((item) => !selectedKeys.has(item.key));
    ...
    return `
      ...
      ${shown.length ? `
        ${selected.length ? '<p class="form-hint manual-mat-merge-group-label">תוצאות (כמו במחסן)</p>' : ''}
  ...
  openModal({
    ...
    onCloseCallback: () => { renderSuppliers(container); },
    bodyHTML: `
      <p class="form-hint" style="margin-top:0;line-height:1.5">
        הרשימה זהה ל<strong>מחסן</strong> — חומר אחד לשם (לא כל ספק בנפרד).
        ${catName ? `מוצג לפי קטגוריה: <strong>${escapeHtml(catName)}</strong>.` : 'מוצג: כל הקטגוריות.'}
        «יעד» = השם שישאר במחסן; שמות אחרים יהפכו למילים נרדפות וייעלמו מהרשימה.
        ספק אחר עם מחיר נשמר כהצעה <em>בתוך</em> חומר היעד (לא כרטיס נפרד במחסן).
        אחרי איחוד החלון נשאר פתוח להמשך.
      </p>
      <div class="form-group" style="margin-bottom:8px">
        <input type="search" id="manual-mat-merge-search" placeholder="חיפוש כמו במחסן..." autocomplete="off">
      </div>
  ...
  function selectedItems() {
    return [...selectedKeys].map((key) => catalogByKey.get(key)).filter(Boolean);
  }
  ...
  function updatePreview() {
    ...
    if (items.length < 2) {
      previewEl.textContent = 'יש לבחור לפחות 2 חומרים מהמחסן · אחרי איחוד אפשר להמשיך מכאן';
      return;
    }
    const keepItem = items.find((i) => i.key === keepKey) || items[0];
    const others = items.filter((i) => i.key !== keepItem.key);
    const keepMat = keepItem.offers.find((o) => o.id === keepIdForItem(keepItem)) || keepItem.offers[0];
    const otherMats = others.flatMap((i) => i.offers);
    const syns = buildMergedMaterialSynonyms(keepMat, otherMats);
    const synText = syns.length ? syns.join(' · ') : others.map((i) => i.name).join(' · ');
    const { preserve } = classifyMaterialsForMerge(keepMat, otherMats);
    previewEl.innerHTML = `יעד במחסן: <strong>${escapeHtml(keepItem.name)}</strong>
      · ייעלמו מהמחסן: <strong>${escapeHtml(others.map((i) => i.name).join(' · '))}</strong>
      · מילים נרדפות: ${escapeHtml(synText || '—')}
      ${preserve.length
    ? `<br>הצעות ספק (${preserve.length}) יישארו <em>בתוך</em> «${escapeHtml(keepItem.name)}» — לא כרטיסים נפרדים`
    : ''}`;
  }
    ...
    listEl.querySelectorAll('.manual-mat-merge-check').forEach((cb) => {
      cb.addEventListener('change', () => {
        ...
        if (cb.checked) {
          ...
          if (keepKey === key) keepKey = selectedKeys.size ? [...selectedKeys][0] : null;
        }
        rerenderList();
        updatePreview();
      });
  ...
  /** רענון הרשימה בחלון אחרי איחוד — בלי לסגור, עם שמירת חיפוש ומיקום גלילה */
  async function refreshAfterMerge({ keptKey, absorbedKeys = [] } = {}) {
    const scrollTop = listEl?.scrollTop || 0;
    const searchValue = searchEl?.value || '';
    const nextCatalog = await getMasterMaterialsList(catId || undefined);
    catalogByKey = new Map(nextCatalog.map((item) => [item.key, item]));
    sorted = [...nextCatalog];

    for (const key of absorbedKeys) selectedKeys.delete(key);
    for (const key of [...selectedKeys]) {
      if (!catalogByKey.has(key)) selectedKeys.delete(key);
    }
    if (keptKey && catalogByKey.has(keptKey)) {
      keepKey = keptKey;
      selectedKeys.add(keptKey);
    } else if (keepKey && !catalogByKey.has(keepKey)) {
      keepKey = selectedKeys.size ? [...selectedKeys][0] : null;
    }
    ...
    if (sorted.length < 2) {
      showToast('אין עוד חומרים לאיחוד ברשימה הזו');
    }
  }
  ...
  document.querySelector('.modal-cancel')?.addEventListener('click', closeMergeSelectedModal);
  ...
  document.getElementById('confirm-manual-mat-merge')?.addEventListener('click', async () => {
    ...
    if (!keepKey || !items.some((i) => i.key === keepKey)) {
      showToast('בחר חומר יעד');
      return;
    }
    const keepItem = items.find((i) => i.key === keepKey);
    const keepId = keepIdForItem(keepItem);
    const absorbedKeys = items.filter((i) => i.key !== keepKey).map((i) => i.key);
    const mergeIds = [];
    for (const item of items) {
      if (item.key === keepKey) continue;
      for (const offer of item.offers) mergeIds.push(offer.id);
    }
    if (!keepId || !mergeIds.length) {
      showToast('אין מה לאחד');
      return;
    }
    ...
    try {
      ...
      const removedNames = items.filter((i) => i.key !== keepKey).map((i) => i.name);
      showToast(preserved
        ? `אוחד ✓ · «${keepItem.name}» · נעלמו: ${removedNames.join(' · ')} · ${preserved} הצעות ספק בפנים`
        : `אוחד ✓ · «${keepItem.name}» · נעלמו: ${removedNames.join(' · ')}`);
      requestAutoBackupNow().catch(() => {});
      await refreshAfterMerge({ keptKey: keepKey, absorbedKeys });
    } catch (err) {
    ...
    <div class="card supplier-browse-intro">
      ...
      <div class="form-group" style="margin:0">
        <input type="search" id="browse-search" class="catalog-search-input"
          placeholder="חיפוש ספק / חומר / מק״ט / ברקוד..." value="${escapeHtml(container.dataset.browseSearch || '')}">
      </div>` : ''}
    </div>
  ...
  return `
    <div class="card supplier-browse-cat">
      <button type="button" class="supplier-browse-cat-header category-toggle-browse">
        <span class="supplier-browse-cat-name">${supplierCategoryIcon(cat)}${escapeHtml(cat.name)}</span>
      ...
      <div class="supplier-browse-cat-body">
        ${cat.suppliers.map((s) => renderBrowseSupplierBlock(s, {
    search, expandedIds, isPackaging, isCleaning, categoryId: cat.id,
  })).join('')}
      </div>
    </div>`;
}
...
function renderBrowseSupplierMaterialsHTML(materials, { isPackaging = false, isCleaning = false } = {}) {
  if (isPackaging || isCleaning) {
    if (!materials.length) return '';
    return `
      <div class="browse-mats-section browse-mats-section--active">
        <div class="browse-mats-section-label">${isCleaning ? 'חומרי ניקיון' : 'אריזות'} (${materials.length})</div>
...
function renderBrowseSupplierBlock(supplier, {
  search, expandedIds, isPackaging = false, isCleaning = false, categoryId = null,
} = {}) {
  ...
  const metaParts = [];
  if (isPackaging || isCleaning) {
    metaParts.push(`${supplier.materials.length} ${isCleaning ? 'חומרי ניקיון' : 'אריזות'}`);
  } else {
...
async function openMaterialDetailModal(container, materialId) {
  ...
  openModal({
    ...
    bodyHTML: `
      ...
      ${sanitizeMaterialNotes(mat.notes) ? `
      <div class="form-group" style="margin-top:10px">
        <label>הערות</label>
        <p class="material-detail-notes">${escapeHtml(sanitizeMaterialNotes(mat.notes))}</p>
      </div>` : ''}
...
async function renderEditSections(host, container, categories, selectedMatCat) {
  ...
  const isCleaningCat = isCleaningSupplierCategory(selectedCategory);
    ...
    <div class="card">
      ...
      <div class="form-group" style="margin-bottom:10px">
        <input type="search" id="edit-mat-search" class="edit-mat-search-input"
          placeholder="חיפוש לפי שם / מק״ט / ברקוד / ספק..."
      ...
      <div class="filter-row" style="margin-bottom:12px">
        <div class="form-hint" style="margin:0;flex:1" id="edit-mat-list-hint">
          ...
          ${isCleaningCat ? ' · קטגוריית חומרי ניקיון 🧹' : (isPackagingCat ? ' · קטגוריית אריזות' : '')}
        ...
        <button type="button" class="btn btn-primary btn-sm" id="add-material">+ ${isCleaningCat ? 'חומר ניקיון' : (isPackagingCat ? 'אריזה' : 'חומר')}</button>
      </div>
...
function openSupplierCategoryForm(container, category) {
  ...
  const kind = category?.isCleaning ? 'cleaning' : (category?.isPackaging ? 'packaging' : 'regular');
  openModal({
    ...
    bodyHTML: `
      ...
      <div class="form-group">
        ...
        <div class="packaging-kind-row" role="radiogroup">
          <label class="packaging-kind-option"><input type="radio" name="sup-cat-kind" value="regular"${kind === 'regular' ? ' checked' : ''}> חומרי גלם</label>
          <label class="packaging-kind-option"><input type="radio" name="sup-cat-kind" value="packaging"${kind === 'packaging' ? ' checked' : ''}> 📦 אריזות</label>
          <label class="packaging-kind-option"><input type="radio" name="sup-cat-kind" value="cleaning"${kind === 'cleaning' ? ' checked' : ''}> 🧹 חומרי ניקיון</label>
        </div>
      </div>
      <p class="form-hint">אריזות: סוג (קרטון/פלסטיק), כמה בחבילה ושיוך למוצר · חומרי ניקיון: מחיר פשוט ליחידה, לא נכנסים למתכונים</p>`,
    footerHTML: `
  ...
  document.getElementById('save-sup-cat')?.addEventListener('click', async () => {
    ...
    const selectedKind = document.querySelector('input[name="sup-cat-kind"]:checked')?.value || 'regular';
    const isPackaging = selectedKind === 'packaging';
    const isCleaning = selectedKind === 'cleaning';
    if (!name) return showToast('הזן שם');
    try {
      if (isEdit) await updateSupplierCategory(category.id, { name, isPackaging, isCleaning });
      else await addSupplierCategory(name, { isPackaging, isCleaning });
      closeModal();
      showToast('נשמר ✓');
      renderSuppliers(container);
    } catch (e) {
...
async function openAddMaterialModal(container, categoryId, suppliers, category, {
  ...
  const seedMat = preferredSupplierId
    ? { supplierId: Number(preferredSupplierId) }
    : null;
  openModal({
    title: isCleaning ? 'חומר ניקיון חדש 🧹' : (isPackaging ? 'אריזה חדשה' : 'חומר גלם חדש'),
    bodyHTML: materialFormHTML(seedMat, suppliers, { isPackaging, isCleaning, products, categories }),
  ...
  if (preferredSupplierId) {
    ...
    if (sel) sel.value = String(preferredSupplierId);
  }
}
...
function materialFormHTML(mat, suppliers, { isPackaging = false, isCleaning = false, products = [], categories = [] } = {}) {
  ...
  const skuBlock = `
    <div class="form-group mat-sku-group">
      ...
      <input type="text" id="mat-sku" autocomplete="off"
        value="${mat ? escapeHtml(sanitizeSku(mat.sku) || '') : ''}"
        placeholder="למשל: FLR-25KG / 12345">
  ...
  const notesMoqBlock = `
    <div class="form-group">
      ...
      <input type="number" id="mat-min-order" min="0" step="0.001"
  ...
  const advancedWrap = (inner) => (isNew
    ? `<details class="mat-advanced-details"><summary>פרטים נוספים (ברקוד, הערות, מילים נרדפות${isPackaging || isCleaning ? '' : ', עיבוד/מנה'}…)</summary>${inner}</details>`
    : inner);
  ...
  if (isCleaning) {
    return `
      ...
      ${cleaningBlock}
      ${supplierBlock}
      ${advancedWrap(`${notesMoqBlock}${barcodeBlock}${synonymsBlock}`)}
      ${recipeDefaultBlock}`;
  }
  if (isPackaging) {
    return `
...
async function renderShortagesTab(body, container) {
  ...
  body.querySelectorAll('.shortage-receive-btn').forEach((btn) => {
    btn.addEventListener('click', async () => {
      const { renderLotPickerFieldHTML, bindLotPickerFields } = await import('../lot-picker.js?v=480');
      openModal({
    ...
    inputHtml: '<input type="text" id="receive-lot-number" placeholder="מספר על האריזה">',
      ...
      document.getElementById('receive-lot-save')?.addEventListener('click', async () => {
        try {
          const { receiveShortageToInventory } = await import('../inventory-db.js?v=480');
  ...
  document.getElementById('receive-open-shortages')?.addEventListener('click', async () => {
    ...
    try {
      const { receiveOpenShortagesToInventory } = await import('../inventory-db.js?v=480');
