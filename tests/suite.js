import { test, testAsync, assertEqual, assertOk, assertApprox, flushTests } from './runner.js?v=415';
import {
  isValidISODate, sanitizeQuantity, sanitizeMoney, sanitizeName, sanitizeRecipeQuantity, roundMoney,
} from '../js/validators.js?v=415';
import {
...
} from '../js/calc.js?v=415';
import { parseDate, parseQuantity, detectAndParse, parseImportFile } from '../js/import.js?v=415';
import { enrichBackupData, summarizeBackupData, formatBackupSummary } from '../js/backup.js?v=415';
import {
...
} from '../js/supplier-import.js?v=415';
import { parseRecipesFromDocumentXml } from '../js/recipe-import.js?v=415';
import { isFlowsReportType, isManagerReportType, normalizeReportType, groupRunsByFlow, filterProductionHistoryEntries, productIdsForHistoryScope, sortProductionHistoryEntries, managerRecordInDateRange, filterManagerTasksByRange } from '../js/screens/reports.js?v=415';
import { runStepsAllCompleted, findNextIncompleteStepIndex, parseNumericBatchNumber, computeNextBatchNumber } from '../js/db.js?v=415';
import { haccpRoleLabel, HACCP_STEPS, evaluateCcpDecisionTree, formatCriticalLimit, haccpMonitorMethodLabel, haccpMonitorFrequencyLabel, haccpProductDispositionLabel, haccpVerificationMethodLabel, haccpVerificationFrequencyLabel, haccpDocKindLabel, haccpDocFormatLabel, haccpPrpTopicLabel, haccpPrpStatusLabel, HACCP_PRP_TOPICS, haccpMonitorLogResultLabel } from '../js/haccp-db.js?v=415';
import { buildHaccpPlanPrintHtml } from '../js/haccp-print.js?v=415';
import { WORKSPACES } from '../js/workspaces.js?v=415';
import { userRoleLabel, userStatusLabel } from '../js/auth.js?v=415';
import { sanitizeAuditPayload } from '../js/audit.js?v=415';
import {
...
export async function runAllTests() {
  ...
  test('permissions — allowedWorkspaces לפי תפקיד + עמדת חשבונות', () => {
    ...
    assertEqual(allowedWorkspaces('manager').sort().join(','), 'accounts,haccp,manager,production,recipes,suppliers');
    assertEqual(allowedWorkspaces('admin').sort().join(','), 'accounts,haccp,manager,production,recipes,suppliers');
  ...
  test('workspaces — עמדת accounts קיימת', () => {
  ...
  test('getBackupScopeId — מזהה קבוע לשחזור אחרי מחיקה', async () => {
    const { getBackupScopeId, BACKUP_SCOPE_ID } = await import('../js/supabase-backup.js?v=415');
