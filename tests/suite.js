import { test, testAsync, assertEqual, assertOk, assertApprox, flushTests } from './runner.js?v=483';
import {
  isValidISODate, sanitizeQuantity, sanitizeMoney, sanitizeName, sanitizeRecipeQuantity, roundMoney,
} from '../js/validators.js?v=483';
import {
...
} from '../js/calc.js?v=483';
import { parseDate, parseQuantity, detectAndParse, parseImportFile } from '../js/import.js?v=483';
import { enrichBackupData, summarizeBackupData, formatBackupSummary } from '../js/backup.js?v=483';
import {
...
} from '../js/supplier-import.js?v=483';
import { parseRecipesFromDocumentXml } from '../js/recipe-import.js?v=483';
import { isFlowsReportType, isManagerReportType, normalizeReportType, groupRunsByFlow, filterProductionHistoryEntries, productIdsForHistoryScope, sortProductionHistoryEntries, managerRecordInDateRange, filterManagerTasksByRange, buildPnlDailyRows, sumProcessSummaryQty } from '../js/screens/reports.js?v=483';
import { runStepsAllCompleted, findNextIncompleteStepIndex, parseNumericBatchNumber, computeNextBatchNumber, isProductWasteCategory, isWasteProductionEntry, computeRunMetrics, PRODUCT_WASTE_CATEGORY_NAME } from '../js/db.js?v=483';
import { haccpRoleLabel, HACCP_STEPS, evaluateCcpDecisionTree, formatCriticalLimit, haccpMonitorMethodLabel, haccpMonitorFrequencyLabel, haccpProductDispositionLabel, haccpVerificationMethodLabel, haccpVerificationFrequencyLabel, haccpDocKindLabel, haccpDocFormatLabel, haccpPrpTopicLabel, haccpPrpStatusLabel, HACCP_PRP_TOPICS, haccpMonitorLogResultLabel, buildHaccpTeamRoleCoverage } from '../js/haccp-db.js?v=483';
import { buildHaccpPlanPrintHtml } from '../js/haccp-print.js?v=483';
import { WORKSPACES } from '../js/workspaces.js?v=483';
import { userRoleLabel, userStatusLabel } from '../js/auth.js?v=483';
import { sanitizeAuditPayload, auditActionLabel, auditEntityLabel, formatAuditSnapshotSummary } from '../js/audit.js?v=483';
import {
  serializeCell, rowsToAoa, uniqueSheetName, listDataTables, buildDataTablesWorkbookSpec, DATA_TABLE_LABELS,
} from '../js/data-tables-export.js?v=483';
import { resolveDownloadFilename } from '../js/download.js?v=483';
import {
...
} from '../js/permissions.js?v=483';
import { lotTraceEmptyHint } from '../js/lot-trace.js?v=483';
import {
...
import {
  ...
  FINANCE_ENCODINGS,
} from '../js/finance-import.js?v=483';
...
export async function runAllTests() {
  ...
  test('חיי מדף ותנאי אחסון — פורמט ופירוש', () => {
    assertEqual(formatProductShelfLife(5, 'day'), '5 ימים');
    assertEqual(formatProductShelfLife(1, 'day'), '1 יום');
    assertEqual(formatProductShelfLife(3, 'month'), '3 חודשים');
    assertEqual(formatProductShelfLife(1, 'year'), '1 שנה');
    assertEqual(formatProductShelfLife('', 'day'), '');
    assertEqual(productStorageConditionLabel('cool'), 'קירור');

    const fromStructured = resolveProductShelfLifeFields({
      shelfLifeValue: 7, shelfLifeUnit: 'day', shelfLife: 'ignore',
    });
    assertEqual(fromStructured.value, 7);
    assertEqual(fromStructured.unit, 'day');

    const fromText = resolveProductShelfLifeFields({ shelfLife: '14 ימים בקירור' });
    assertEqual(fromText.value, 14);
    assertEqual(fromText.unit, 'day');

    assertEqual(resolveProductStorageConditionId({ storageConditionId: 'frozen' }), 'frozen');
    assertEqual(resolveProductStorageConditionId({ storageConditions: 'קירור' }), 'cool');
    assertEqual(resolveProductStorageConditionId({ storageConditions: 'הקפאה עמוקה' }), 'frozen');
  });
  ...
  test('addMaterialUsageToMap — combines grams and kg of the same material', () => {
    const map = new Map();
    addMaterialUsageToMap(map, { name: 'קמח', quantity: 500, unitKind: 'g', rawMaterialId: 7 });
    addMaterialUsageToMap(map, { name: 'קמח', scaledQuantity: 2, unitKind: 'kg', rawMaterialId: 7 });
    const row = map.get('id:7:kg');
    assertOk(row);
    assertApprox(row.totalQty, 2.5);
    assertEqual(row.unitKind, 'kg');
    assertEqual(formatMaterialUsageQty(row.totalQty, row.unitKind), '2.5 ק"ג');
  });

  test('addMaterialUsageToMap — liters stay separate from weight', () => {
    const map = new Map();
    addMaterialUsageToMap(map, { name: 'מים', quantity: 2, unitKind: 'l' });
    addMaterialUsageToMap(map, { name: 'מים', quantity: 1, unitKind: 'kg' });
    assertEqual(map.size, 2);
    const water = [...map.values()].find((r) => r.unitKind === 'l');
    assertEqual(formatMaterialUsageQty(water.totalQty, 'l'), '2 ליטר');
  });

  test('groupMaterialUsageByCategory — sorts categories and items in Hebrew', () => {
    const grouped = groupMaterialUsageByCategory([
      { name: 'סוכר', supplierCategoryId: 2, supplierCategoryName: 'יבשים', totalQty: 1, unitKind: 'kg' },
      { name: 'קמח', supplierCategoryId: 2, supplierCategoryName: 'יבשים', totalQty: 3, unitKind: 'kg' },
      { name: 'חלב', supplierCategoryId: 1, supplierCategoryName: 'חלב', totalQty: 4, unitKind: 'l' },
    ]);
  ...
  test('ingredientBelongsToRecipeVersion — untagged goes to first version, not current default', () => {
    const versions = [
      { id: 1, name: 'גרסה 1', sortOrder: 1, isDefault: false },
      { id: 2, name: 'גרסה 2', sortOrder: 2, isDefault: true },
    ];
    const untagged = { id: 10, name: 'קמח', recipeVersionId: null };
    const v2ing = { id: 11, name: 'קמח', recipeVersionId: 2 };
    assertEqual(homeRecipeVersionId(versions), 1);
    assertOk(ingredientBelongsToRecipeVersion(untagged, versions[0], versions));
    assertOk(!ingredientBelongsToRecipeVersion(untagged, versions[1], versions));
    assertOk(ingredientBelongsToRecipeVersion(v2ing, versions[1], versions));
    assertOk(!ingredientBelongsToRecipeVersion(v2ing, versions[0], versions));
    assertOk(isUntaggedRecipeVersionId(null));
    assertOk(isUntaggedRecipeVersionId(''));
    assertOk(sameRecipeVersionId(2, '2'));
    assertOk(!sameRecipeVersionId(1, 2));
  });

  test('planRecipeVersionIngredientRepair — doubled v2 + empty v1 splits one copy back', () => {
    const versions = [
      { id: 1, name: 'גרסה 1', sortOrder: 1, isDefault: false },
      { id: 2, name: 'גרסה 2', sortOrder: 2, isDefault: true },
    ];
    const ingredients = [
      { id: 101, name: 'קמח', quantity: 12, recipeVersionId: 2, sortOrder: 1 },
      { id: 102, name: 'סוכר', quantity: 7.5, recipeVersionId: 2, sortOrder: 2 },
      { id: 103, name: 'דבש טבעי', quantity: 500, recipeVersionId: 2, sortOrder: 3 },
      { id: 201, name: 'קמח', quantity: 10, recipeVersionId: 2, sortOrder: 4 },
      { id: 202, name: 'סוכר', quantity: 6, recipeVersionId: 2, sortOrder: 5 },
      { id: 203, name: 'דבש טבעי', quantity: 400, recipeVersionId: 2, sortOrder: 6 },
    ];
    const plan = planRecipeVersionIngredientRepair(versions, ingredients);
    assertEqual(plan.retags.length, 0);
    assertEqual(plan.moves.length, 3);
    const dests = new Set(plan.moves.map((m) => m.toVersionId));
    assertEqual(dests.size, 1);
    assertEqual([...dests][0], 1);
    const movedIds = plan.moves.map((m) => m.ingredientId).sort((a, b) => a - b);
    assertEqual(movedIds.join(','), '201,202,203');
    assertEqual((plan.deletes || []).length, 0);
  });

  test('planRecipeVersionIngredientRepair — untagged leftovers retag to version 1', () => {
    const versions = [
      { id: 1, name: 'גרסה 1', sortOrder: 1, isDefault: false },
      { id: 2, name: 'גרסה 2', sortOrder: 2, isDefault: true },
    ];
    const ingredients = [
      { id: 1, name: 'קמח', recipeVersionId: null, sortOrder: 1 },
      { id: 2, name: 'סוכר', recipeVersionId: '', sortOrder: 2 },
      { id: 3, name: 'קמח', recipeVersionId: 2, sortOrder: 1 },
      { id: 4, name: 'סוכר', recipeVersionId: 2, sortOrder: 2 },
    ];
    const plan = planRecipeVersionIngredientRepair(versions, ingredients);
    assertEqual(plan.retags.length, 2);
    assertOk(plan.retags.every((r) => r.toVersionId === 1));
    assertEqual(plan.moves.length, 0, 'after treating untagged as v1 there is no leftover duplicate to move');
    assertEqual((plan.deletes || []).length, 0);
  });

  test('planRecipeVersionIngredientRepair — same names on both versions stay put', () => {
    const versions = [
      { id: 1, name: 'גרסה 1', sortOrder: 1, isDefault: true },
      { id: 2, name: 'גרסה 2', sortOrder: 2, isDefault: false },
    ];
    const ingredients = [
      { id: 1, name: 'קמח', quantity: 12, recipeVersionId: 1, sortOrder: 1 },
      { id: 2, name: 'קמח', quantity: 10, recipeVersionId: 2, sortOrder: 1 },
    ];
    const plan = planRecipeVersionIngredientRepair(versions, ingredients);
    assertEqual(plan.retags.length, 0);
    assertEqual(plan.moves.length, 0);
    assertEqual((plan.deletes || []).length, 0);
  });

  test('planRecipeVersionIngredientRepair — v2 doubled while v1 still full deletes extras', () => {
    const versions = [
      { id: 1, name: 'גרסה 1', sortOrder: 1, isDefault: true },
      { id: 2, name: 'גרסה 2', sortOrder: 2, isDefault: false },
    ];
    const ingredients = [
      { id: 1, name: 'קמח', quantity: 12, unitKind: 'kg', recipeVersionId: 1, sortOrder: 1 },
      { id: 2, name: 'סוכר', quantity: 7.5, unitKind: 'kg', recipeVersionId: 1, sortOrder: 2 },
      { id: 11, name: 'קמח', quantity: 12, unitKind: 'kg', recipeVersionId: 2, sortOrder: 1 },
      { id: 12, name: 'סוכר', quantity: 7.5, unitKind: 'kg', recipeVersionId: 2, sortOrder: 2 },
      { id: 21, name: 'קמח', quantity: 12, unitKind: 'kg', recipeVersionId: 2, sortOrder: 3 },
      { id: 22, name: 'סוכר', quantity: 7.5, unitKind: 'kg', recipeVersionId: 2, sortOrder: 4 },
    ];
    const plan = planRecipeVersionIngredientRepair(versions, ingredients);
    assertEqual(plan.moves.length, 0);
    assertEqual(plan.deletes.sort((a, b) => a - b).join(','), '21,22');
  });

  test('planRecipeVersionIngredientRepair — v2 has copy of v1 plus different qty keeps unique line', () => {
    const versions = [
      { id: 1, name: 'גרסה 1', sortOrder: 1, isDefault: true },
      { id: 2, name: 'גרסה 2', sortOrder: 2, isDefault: false },
    ];
    const ingredients = [
      { id: 1, name: 'קמח', quantity: 12, unitKind: 'kg', recipeVersionId: 1, sortOrder: 1 },
      { id: 11, name: 'קמח', quantity: 12, unitKind: 'kg', recipeVersionId: 2, sortOrder: 1 },
      { id: 12, name: 'קמח', quantity: 10, unitKind: 'kg', recipeVersionId: 2, sortOrder: 2 },
    ];
    const plan = planRecipeVersionIngredientRepair(versions, ingredients);
    assertEqual(plan.moves.length, 0);
    assertEqual(plan.deletes.join(','), '11');
  });
  ...
  await testAsync(
    'computeRecipeMaterialsCostFiltered — עלות מומלצת לא מתאפסת/חלקית כשאין חומר מסומן "ספק"',
    async () => {
      // תרחיש: שני מרכיבים נוספו בזרימה הרגילה (priceSource:'max', rawMaterialId:null —
      // כמו js/screens/recipes.js add-ingredient) בלי סימון ספק ידני לאף אחד מהם.
      const materials = [
        { id: 1, name: 'קמח', unitPrice: 4.5, packageWeightGrams: 1000 },
        { id: 2, name: 'קמח', unitPrice: 5.2, packageWeightGrams: 1000 },
        { id: 3, name: 'סוכר', unitPrice: 6, packageWeightGrams: 1000 },
      ];
      const ingredients = [
        { name: 'קמח', quantity: 1000, unitKind: 'g', priceSource: 'max', rawMaterialId: null },
        { name: 'סוכר', quantity: 500, unitKind: 'g', priceSource: 'max', rawMaterialId: null },
      ];
      const fullCost = await computeRecipeMaterialsCostFiltered(ingredients, materials, { supplierOnly: false });
      const recommendedCost = await computeRecipeMaterialsCostFiltered(ingredients, materials, { supplierOnly: true });
      assertEqual(fullCost, 8.2, 'עלות מלאה: (1kg*5.2) + (0.5kg*6) = 8.2');
      assertEqual(
        recommendedCost,
        fullCost,
        'עלות מומלצת לא נופלת ל-0/חלקית כשאין סימון ספק ידני — משתמשת באותו מחיר אוטומטי',
      );

      // מרכיב אחד מוצמד ידנית לספק זול יותר — שתי הצורות מכבדות את ההצמדה (כמו
      // resolveRecipeIngredientMaterial בכל מקום אחר באפליקציה), ולכן שוות זו לזו.
      const pinnedIngredients = [
        { name: 'קמח', quantity: 1000, unitKind: 'g', priceSource: 'supplier', rawMaterialId: 1 },
        { name: 'סוכר', quantity: 500, unitKind: 'g', priceSource: 'max', rawMaterialId: null },
      ];
      const fullCostPinned = await computeRecipeMaterialsCostFiltered(pinnedIngredients, materials, { supplierOnly: false });
      const recommendedCostPinned = await computeRecipeMaterialsCostFiltered(pinnedIngredients, materials, { supplierOnly: true });
      assertEqual(fullCostPinned, 7.5, 'עלות מלאה: (1kg*4.5 מהצעה מוצמדת) + (0.5kg*6) = 7.5');
      assertEqual(recommendedCostPinned, 7.5, 'עלות מומלצת זהה לעלות המלאה');
    },
  );
  ...
  test('detectImportPriceBasis — מזהה מחירון שרשם מחיר לק"ג', () => {
    ...
    const analysis = analyzeImportPriceBasis(perKgSheet);
    assertEqual(analysis.basis, PRICE_BASIS_PER_KG);
    assertOk(['high', 'medium', 'low'].includes(analysis.confidence));
  });

  test('flagImportEntriesForReview — מסמן שורות בלי משקל או חריגות', () => {
    const entries = [
      { materialName: 'קמח', supplierName: 'א', price: 50, packageWeightGrams: 25000 },
      { materialName: 'בלי משקל', supplierName: 'א', price: 12, packageWeightGrams: null },
      { materialName: 'יקר מוזר', supplierName: 'ב', price: 9000, packageWeightGrams: 1000 },
    ];
    const flagged = flagImportEntriesForReview(entries, PRICE_BASIS_PACKAGE);
    assertOk(flagged.some((f) => f.materialName === 'בלי משקל'));
    assertOk(flagged.some((f) => f.materialName === 'יקר מוזר'));
  });

  test('classifyLiveSyncError — מבחין RLS מממתין ומבעיית רשת', () => {
  ...
  test('sanitizeSku / notes / MOQ — פרופיל חומר', () => {
    assertEqual(sanitizeSku('  FLR-25  '), 'FLR-25');
    assertEqual(sanitizeSku('   '), null);
    assertEqual(sanitizeMaterialNotes('  הערה  '), 'הערה');
    assertEqual(sanitizeMaterialNotes(''), null);
    assertEqual(sanitizeMinOrderQty('2.5'), 2.5);
    assertEqual(sanitizeMinOrderQty(0), null);
    assertEqual(sanitizeMinOrderQty(-1), null);
  });

  test('materialMatchesSearch — מק״ט והערות', () => {
    const mat = { name: 'קמח', sku: 'FLR-01', notes: 'שק 25', barcode: '7290001' };
    assertOk(materialMatchesSearch(mat, 'flr'));
    assertOk(materialMatchesSearch(mat, 'שק'));
    assertOk(materialMatchesSearch(mat, '729'));
    assertOk(!materialMatchesSearch(mat, 'סוכר'));
  });

  test('materialFieldFillPatch — ממלא מק״ט/הערות/MOQ חסרים', () => {
    const keep = { id: 1, name: 'קמח', sku: null, notes: null, minOrderQty: null };
    const patch = materialFieldFillPatch(keep, [
      { id: 2, sku: 'K-1', notes: 'קר', minOrderQty: 5 },
    ]);
    assertEqual(patch.sku, 'K-1');
    assertEqual(patch.notes, 'קר');
    assertEqual(patch.minOrderQty, 5);
  });
  ...
  test('supplierCategoryRoleKey — אריזות/ניקיון/חומ״ג גם בשם שונה', () => {
    ...
    assertEqual(supplierCategoryRoleKey({ name: 'חומרי גלם' }), 'raw');
    assertEqual(supplierCategoryRoleKey({ name: 'חומרי גלם יבשים' }), 'raw');
    assertEqual(supplierCategoryRoleKey({ name: 'ייבוא ממתכונים' }), 'import');
    assertEqual(supplierCategoryRoleKey({ name: 'יבוא ממתכון' }), 'import');
    assertEqual(supplierCategoryRoleKey({ name: 'חלב ומוצריו' }), null);
  });

  test('inferRawMaterialSupplierRole — אריזות רק עם packagingKind, אחרת חומ״ג', () => {
    const packCat = new Map([[9, { id: 9, name: 'אריזות', isPackaging: true }]]);
    assertEqual(inferRawMaterialSupplierRole({ name: 'קמח', supplierCategoryId: 9 }, packCat), 'raw');
    assertEqual(
      inferRawMaterialSupplierRole({ name: 'קמח', packagingKind: 'carton', supplierCategoryId: 9 }, packCat),
      'raw',
      'packagingKind אוטומטי על שם מזון → חומ״ג',
    );
    assertEqual(
      inferRawMaterialSupplierRole({ name: 'קופסת קרטון', packagingKind: 'carton', supplierCategoryId: 9 }, packCat),
      'packaging',
    );
    assertEqual(inferRawMaterialSupplierRole({ name: 'סבון כלים', supplierCategoryId: 9 }, packCat), 'cleaning');
  });

  test('compareSupplierCategories — חומרי גלם, אריזות, חומרי ניקיון', () => {
    const cats = [
      { id: 3, name: 'חומרי ניקיון', isCleaning: true, sortOrder: 1 },
      { id: 2, name: 'אריזות', isPackaging: true, sortOrder: 2 },
      { id: 1, name: 'חומרי גלם', sortOrder: 9 },
      { id: 4, name: 'חלב ומוצריו', sortOrder: 3 },
      { id: 5, name: 'ייבוא ממתכונים', sortOrder: 1 },
    ];
    cats.sort(compareSupplierCategories);
    assertEqual(
      cats.map((c) => c.name).join('|'),
      'חומרי גלם|אריזות|חומרי ניקיון|ייבוא ממתכונים|חלב ומוצריו',
    );
  });

  test('rowDedupeFingerprint — חומר גלם מתעלם מקטגוריית ספק', () => {
    const a = rowDedupeFingerprint('rawMaterials', {
      name: 'קמח', supplierId: 3, supplierCategoryId: 1,
    });
    const b = rowDedupeFingerprint('rawMaterials', {
      name: 'קמח', supplierId: 3, supplierCategoryId: 99,
    });
    assertEqual(a, b, 'same name+supplier under different categories is a duplicate');
    const c = rowDedupeFingerprint('rawMaterials', {
      name: 'קמח', supplierId: 4, supplierCategoryId: 1,
    });
    assertOk(a !== c, 'different supplier is a different material');
    // pull-match עדיין מבחין לפי קטגוריה
    const pullA = rowFingerprint('rawMaterials', {
      name: 'קמח', supplierId: 3, supplierCategoryId: 1,
    });
    const pullB = rowFingerprint('rawMaterials', {
      name: 'קמח', supplierId: 3, supplierCategoryId: 99,
    });
    assertOk(pullA !== pullB, 'pull fingerprint still includes category');
  });

  test('sameNumericId — מחרוזת מול מספר אחרי סנכרון', () => {
    assertOk(sameNumericId('12', 12));
    assertOk(sameNumericId(12, 12));
    assertOk(!sameNumericId('12', 13));
    assertOk(!sameNumericId(null, 1));
    assertOk(!sameNumericId('', 1));
  });

  test('rowFingerprint — מזהי ספק/קטגוריה כמחרוזת מתלכדים למספר', () => {
    const a = rowFingerprint('rawMaterials', {
      name: 'קמח', supplierId: '3', supplierCategoryId: '1',
    });
    const b = rowFingerprint('rawMaterials', {
      name: 'קמח', supplierId: 3, supplierCategoryId: 1,
    });
    assertEqual(a, b);
    const sa = rowFingerprint('suppliers', { name: 'מאפיית על', categoryId: '9' });
    const sb = rowFingerprint('suppliers', { name: 'מאפיית על', categoryId: 9 });
    assertEqual(sa, sb);
  });

  test('rowDedupeFingerprint — בלי ספק לא ממזגים בין קטגוריות', () => {
    const a = rowDedupeFingerprint('rawMaterials', {
      name: 'שמן', supplierId: null, supplierCategoryId: 1,
    });
    const b = rowDedupeFingerprint('rawMaterials', {
      name: 'שמן', supplierId: null, supplierCategoryId: 2,
    });
    assertOk(a !== b, 'שני שמנים בלי ספק בקטגוריות שונות נשארים נפרדים');
    const c = rowDedupeFingerprint('rawMaterials', {
      name: 'שמן', supplierId: '', supplierCategoryId: 1,
    });
    assertEqual(a, c);
  });

  test('rowDedupeFingerprint — קטגוריית ספק לפי תפקיד (אריזה/ניקיון/חומ״ג) גם בשם שונה', () => {
    const a = rowDedupeFingerprint('supplierCategories', { name: 'אריזה', isPackaging: true });
    const b = rowDedupeFingerprint('supplierCategories', { name: 'אריזות', isPackaging: false });
    assertEqual(a, b, 'same packaging role under different names is a duplicate');
    const cleaning = rowDedupeFingerprint('supplierCategories', { name: 'חומרי ניקיון', isCleaning: true });
    assertOk(a !== cleaning, 'packaging and cleaning roles must not collapse');
    // חומ״ג — גם seed «יבשים» מתמזג עם «חומרי גלם»
    const rawA = rowDedupeFingerprint('supplierCategories', { name: 'חומרי גלם' });
    const rawB = rowDedupeFingerprint('supplierCategories', { name: 'חומרי גלם יבשים' });
    assertEqual(rawA, rawB, 'raw materials category variants must merge');
    const dairy = rowDedupeFingerprint('supplierCategories', { name: 'חלב ומוצריו' });
    assertOk(rawA !== dairy, 'other categories still require an exact name match');
    const importA = rowDedupeFingerprint('supplierCategories', { name: 'ייבוא ממתכונים' });
    const importB = rowDedupeFingerprint('supplierCategories', { name: 'יבוא ממתכון' });
    assertEqual(importA, importB, 'import-from-recipes variants must merge');
    assertOk(importA !== rawA, 'import role must not collapse into raw');
    // pull-match (rowFingerprint) עדיין מבחין לפי שם מדויק
    const pullA = rowFingerprint('supplierCategories', { name: 'אריזה' });
    const pullB = rowFingerprint('supplierCategories', { name: 'אריזות' });
    assertOk(pullA !== pullB, 'strict pull fingerprint still distinguishes by exact name');
  });
  ...
  test('rowDedupeFingerprint — רישומי ייצור שומרים runId', () => {
    const withRun = rowDedupeFingerprint('productionEntries', {
      date: '2026-07-21', productId: 5, quantity: 1050, runId: 9,
    });
    const otherRun = rowDedupeFingerprint('productionEntries', {
      date: '2026-07-21', productId: 5, quantity: 1050, runId: 12,
    });
    const nullRun = rowDedupeFingerprint('productionEntries', {
      date: '2026-07-21', productId: 5, quantity: 1050, runId: null,
    });
    assertEqual(withRun, 'productionEntries|2026-07-21|5|1050|0|9');
    assertOk(withRun !== otherRun, 'two runs of the same qty stay distinct');
    assertOk(withRun !== nullRun, 'null-run twin does not collapse via generic dedupe');
    assertEqual(
    ...
    assertOk(
      rowDedupeFingerprint('productionEntries', {
        date: '2026-07-21', productId: 5, quantity: 1050, isWaste: true,
      }) !== rowDedupeFingerprint('productionEntries', {
        date: '2026-07-21', productId: 5, quantity: 1050, isWaste: false,
      }),
      'good and waste entries do not collapse',
    );
  });

  test('rowFingerprint — תזרימי ייצור נבדלים לפי startedAt', () => {
    const a = rowFingerprint('productionRuns', {
      date: '2026-08-20', batchNumber: '12', flowId: 3, startedAt: '2026-08-20T07:00:00',
    });
    const b = rowFingerprint('productionRuns', {
      date: '2026-08-20', batchNumber: '12', flowId: 3, startedAt: '2026-08-20T15:00:00',
    });
    assertEqual(a, 'productionRuns|2026-08-20|12|3|2026-08-20T07:00:00');
    assertOk(a !== b, 'two same-day flows of the same batch stay distinct');
  });

  test('ARRAY_FKS — productionEntryIds משויך לרישומי ייצור', () => {
    assertEqual(ARRAY_FKS.runStepStates.productionEntryIds, 'productionEntries');
  });

  test('mergeRemoteTransactionalRow — לא פותח מחדש תזרים שהושלם', () => {
    const merged = mergeRemoteTransactionalRow(
      'productionRuns',
      { status: 'completed', completedAt: '2026-08-20T18:00:00', currentStepIndex: 4 },
      { status: 'active', completedAt: null, currentStepIndex: 2, flowName: 'עוגות' },
    );
    assertEqual(merged.status, 'completed');
    assertEqual(merged.completedAt, '2026-08-20T18:00:00');
    assertEqual(merged.currentStepIndex, 4);
    assertEqual(merged.flowName, 'עוגות');
  });

  test('mergeRemoteTransactionalRow — מאחד productionEntryIds ולא מוחק מקומיים', () => {
    const merged = mergeRemoteTransactionalRow(
      'runStepStates',
      { status: 'completed', productionEntryIds: [11, 12], completedAt: '2026-08-20T18:00:00' },
      { status: 'pending', productionEntryIds: [], completedAt: null },
    );
    assertEqual(merged.status, 'completed');
    assertEqual(merged.productionEntryIds.join(','), '11,12');
    assertEqual(merged.completedAt, '2026-08-20T18:00:00');
  });
  ...
  test('inventoryMovementKindLabel', async () => {
    const { inventoryMovementKindLabel, formatWhatsAppGapOrderText } = await import('../js/inventory-db.js?v=483');
  ...
  test('receiveShortageToInventory — דורש מזהה', async () => {
    const { receiveShortageToInventory } = await import('../js/inventory-db.js?v=483');
    let threw = false;
    try {
  ...
  test('previewProductionStockIssue — דורש מספר מנות', async () => {
    const { previewProductionStockIssue, formatProductionIssueConfirm } = await import('../js/inventory-db.js?v=483');
    let threw = false;
    try {
  ...
  test('HACCP — כלי בניית תכנית מיוצאים', async () => {
    const mod = await import('../js/haccp-db.js?v=483');
  ...
  test('isWasteProductionRecord / isProductWasteCategory', () => {
    assertOk(isProductWasteCategory({ name: PRODUCT_WASTE_CATEGORY_NAME, isProductWaste: true }));
    assertOk(isProductWasteCategory({ name: 'פחת מוצר' }));
    assertOk(!isProductWasteCategory({ name: 'שטרודל' }));
    assertOk(isWasteProductionRecord({ isWaste: true }, { id: 1 }));
    assertOk(!isWasteProductionRecord({ quantity: 5 }, { id: 1 }));
    assertOk(isWasteProductionEntry({ isWaste: true }));
    assertOk(isWasteProductionEntry({}, { categoryId: 9 }, { id: 9, name: 'פחת מוצר' }));
    assertOk(!isWasteProductionEntry({ quantity: 4 }, { name: 'פרג' }, { name: 'שטרודל' }));
  });

  test('computeProductionTotals — פחת לא נספר במכירה אבל עלות נשארת', () => {
    const productMap = new Map([[1, { id: 1, categoryId: 10, unitPrice: 8, rawMaterialsCost: 3 }]]);
    const t = computeProductionTotals([
      { productId: 1, quantity: 10 },
      { productId: 1, quantity: 2, isWaste: true },
    ], productMap);
    assertEqual(t.total, 10);
    assertApprox(t.totalValue, 80);
    assertEqual(t.wasteQty, 2);
    assertApprox(t.wasteCost, 6);
    assertApprox(t.totalCost, 36);
  });

  test('computeRunMetrics — ייצור נטו אחרי פחת', () => {
    const m = computeRunMetrics({ steps: [], status: 'completed' }, [
      { productId: 1, quantity: 20 },
      { productId: 1, quantity: 5, isWaste: true },
    ]);
    assertEqual(m.productionQty, 20);
    assertEqual(m.wasteQty, 5);
    assertEqual(m.grossProductionQty, 25);
    assertEqual(m.productionByProduct.get(1), 20);
    assertEqual(m.wasteByProduct.get(1), 5);
  });

  test('metricsProductionValueBreakdown — ערך לפי מוצר בלי פחת', () => {
    const productMap = buildProductMap([
      { id: 1, name: 'פרג', unitPrice: 10 },
      { id: 2, name: 'גבינה', unitPrice: 8 },
    ]);
    const metrics = computeRunMetrics({ steps: [], status: 'completed' }, [
      { productId: 1, quantity: 3 },
      { productId: 2, quantity: 5 },
      { productId: 1, quantity: 2, isWaste: true },
    ]);
    const r = metricsProductionValueBreakdown(metrics, productMap);
    assertEqual(r.rows.length, 2);
    assertApprox(r.rows.find((x) => x.productId === 1).value, 30);
    assertApprox(r.rows.find((x) => x.productId === 2).value, 40);
    assertApprox(r.totalValue, 70);
  });

  test('metricsProductionValueBreakdown — מחיר לק״ג יחידות', () => {
    const productMap = buildProductMap([
      { id: 9, name: 'לחמניה', unitPrice: 20, priceUnit: 'kg_units', unitWeightKg: 0.5 },
    ]);
    const metrics = { productionByProduct: new Map([[9, 4]]) };
    const r = metricsProductionValueBreakdown(metrics, productMap);
    assertApprox(productLineValue(productMap.get(9), 4), 40);
    assertApprox(r.totalValue, 40);
  });

  test('auditProductionData — ייצור ופחת באותו יום אינם כפילות', () => {
    const products = [{ id: 1, categoryId: 1, name: 'פרג', unitPrice: 5, rawMaterialsCost: 1 }];
    const entries = [
      { id: 1, date: '2026-06-01', productId: 1, quantity: 10 },
      { id: 2, date: '2026-06-01', productId: 1, quantity: 2, isWaste: true },
    ];
    const audit = auditProductionData(products, entries, [{ id: 1, name: 'שטרודל' }]);
    assertOk(!audit.issues.some((i) => i.kind === 'duplicate_date_product'));
    assertOk(!audit.issues.some((i) => i.kind === 'raw_qty_mismatch'));
  });
  ...
  test('sumProcessSummaryQty — תואם את שורות הפירוט (לא סכימה גולמית)', () => {
    const catMap = new Map([[1, 'מאפינס']]);
    // כמות שלילית (0 אחרי computeProcessSummary) + כמות עם שבר (מעוגלת) — הסכום הראשי
    // חייב להתאים לטבלת הפירוט מתחתיו, לא לסכום הגולמי של processLogs.
    const rows = computeProcessSummary([
      { categoryId: 1, activity: 'עיבוד', quantity: -4 },
      { categoryId: 1, activity: 'עיבוד', quantity: 15.6 },
    ], catMap);
    assertEqual(sumProcessSummaryQty(rows), 16, '0 (מ־-4) + 16 (עיגול 15.6) = 16');
  });

  test('buildPnlDailyRows — ערך הכנסה נכון למוצר kg_units (לא מתעלם ממשקל יחידה)', () => {
    const productMap = buildProductMap([
      {
        id: 1, name: 'עוגת גבינה', categoryId: 1,
        priceUnit: 'kg_units', unitWeightKg: 1.2, unitPrice: 45,
        rawMaterialsCost: 10, packagingCost: 2, additionalCosts: 0,
      },
    ]);
    const rows = buildPnlDailyRows([
      { date: '2026-08-10', productId: 1, quantity: 8 },
    ], productMap);
    assertEqual(rows.length, 1);
    assertEqual(rows[0].qty, 8);
    // 8 יח' * 1.2 ק"ג/יח' * 45 ₪/ק"ג = 432, לא 45*8=360
    assertEqual(rows[0].value, 432, 'הכנסה כוללת משקל יחידה, לא רק unitPrice*qty');
    assertEqual(rows[0].cost, 96, 'עלות: 8 * (10+2)');
    assertEqual(rows[0].margin, 336);
  });
  ...
  test('getBackupScopeId — מזהה קבוע לשחזור אחרי מחיקה', async () => {
    const { getBackupScopeId, BACKUP_SCOPE_ID } = await import('../js/supabase-backup.js?v=483');
...
<w:tr><w:tc><w:p><w:r><w:t>8 ק"ג</w:t></w:r></w:p></w:tc><w:tc><w:p><w:r><w:t>קמח</w:t></w:r></w:p></w:tc></w:tr>
  ...
  test('resolveDownloadFilename — שומר סיומת קיימת', () => {
    assertEqual(resolveDownloadFilename('yitzur-tables-2026-08-18.xlsx'), 'yitzur-tables-2026-08-18.xlsx');
    assertEqual(resolveDownloadFilename('yitzur-gibuy-2026-08-18.json'), 'yitzur-gibuy-2026-08-18.json');
    assertEqual(resolveDownloadFilename('dugma-yitzur.csv'), 'dugma-yitzur.csv');
  });

  test('resolveDownloadFilename — מוסיף סיומת לפי סוג הקובץ', () => {
    assertEqual(resolveDownloadFilename('yitzur-gibuy'), 'yitzur-gibuy.json');
    assertEqual(
      resolveDownloadFilename('yitzur-tables', { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }),
      'yitzur-tables.xlsx',
    );
  });

  test('serializeCell — ערכים פשוטים ומקוננים', () => {
    assertEqual(serializeCell(null), '');
    assertEqual(serializeCell(true), 'כן');
    assertEqual(serializeCell(false), 'לא');
    assertEqual(serializeCell(12.5), 12.5);
    assertEqual(serializeCell('שטרודל'), 'שטרודל');
    assertEqual(serializeCell({ a: 1 }), '{"a":1}');
    assertEqual(serializeCell(['x', 'y']), '["x","y"]');
    assertEqual(serializeCell('data:image/png;base64,AAA', 'imageDataUrl'), 'כן');
  });

  test('rowsToAoa — כותרות, שורות ואיחוד מפתחות', () => {
    const aoa = rowsToAoa([
      { id: 1, name: 'פרג' },
      { id: 2, name: 'גבינה', active: true },
    ]);
    assertEqual(aoa[0].join(','), 'id,name,active');
    assertEqual(aoa[1][0], 1);
    assertEqual(aoa[1][1], 'פרג');
    assertEqual(aoa[1][2], '');
    assertEqual(aoa[2][2], 'כן');
    assertEqual(rowsToAoa([])[0][0], 'אין שורות בטבלה זו');
  });

  test('uniqueSheetName — חיתוך לייחודיות עד 31 תווים', () => {
    const used = new Set();
    const a = uniqueSheetName('קבוצות קטגוריה', used);
    const b = uniqueSheetName('קבוצות קטגוריה', used);
    assertEqual(a, 'קבוצות קטגוריה');
    assertEqual(b, 'קבוצות קטגוריה 2');
    const long = uniqueSheetName('א'.repeat(40), new Set());
    assertEqual(long.length, 31);
  });

  test('buildDataTablesWorkbookSpec — תוכן עניינים + גיליון לכל טבלה', () => {
    const specs = buildDataTablesWorkbookSpec({
      products: [{ id: 1, name: 'פרג 30' }],
      categories: [],
    }, { exportedAt: '18.8.2026', appVersion: 'test' });
    assertEqual(specs[0].sheetName, 'תוכן עניינים');
    assertEqual(specs[0].aoa[1][1], 'test');
    const productSheet = specs.find((s) => s.key === 'products');
    const categorySheet = specs.find((s) => s.key === 'categories');
    assertOk(productSheet);
    assertOk(categorySheet);
    assertEqual(productSheet.rowCount, 1);
    assertEqual(categorySheet.rowCount, 0);
