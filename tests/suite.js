import { test, testAsync, assertEqual, assertOk, assertApprox, flushTests } from './runner.js?v=485';
import {
  isValidISODate, sanitizeQuantity, sanitizeMoney, sanitizeName, sanitizeRecipeQuantity, roundMoney,
} from '../js/validators.js?v=485';
import {
...
} from '../js/calc.js?v=485';
import { parseDate, parseQuantity, detectAndParse, parseImportFile } from '../js/import.js?v=485';
import { enrichBackupData, summarizeBackupData, formatBackupSummary } from '../js/backup.js?v=485';
import {
...
} from '../js/supplier-import.js?v=485';
import { parseRecipesFromDocumentXml } from '../js/recipe-import.js?v=485';
import { isFlowsReportType, isManagerReportType, normalizeReportType, groupRunsByFlow, filterProductionHistoryEntries, productIdsForHistoryScope, sortProductionHistoryEntries, managerRecordInDateRange, filterManagerTasksByRange, buildPnlDailyRows, sumProcessSummaryQty } from '../js/screens/reports.js?v=485';
import { runStepsAllCompleted, findNextIncompleteStepIndex, parseNumericBatchNumber, computeNextBatchNumber, isProductWasteCategory, isWasteProductionEntry, computeRunMetrics, PRODUCT_WASTE_CATEGORY_NAME } from '../js/db.js?v=485';
import { haccpRoleLabel, HACCP_STEPS, evaluateCcpDecisionTree, formatCriticalLimit, haccpMonitorMethodLabel, haccpMonitorFrequencyLabel, haccpProductDispositionLabel, haccpVerificationMethodLabel, haccpVerificationFrequencyLabel, haccpDocKindLabel, haccpDocFormatLabel, haccpPrpTopicLabel, haccpPrpStatusLabel, HACCP_PRP_TOPICS, haccpMonitorLogResultLabel, buildHaccpTeamRoleCoverage } from '../js/haccp-db.js?v=485';
import { buildHaccpPlanPrintHtml } from '../js/haccp-print.js?v=485';
import { WORKSPACES } from '../js/workspaces.js?v=485';
import { userRoleLabel, userStatusLabel } from '../js/auth.js?v=485';
import { sanitizeAuditPayload, auditActionLabel, auditEntityLabel, formatAuditSnapshotSummary } from '../js/audit.js?v=485';
import {
  serializeCell, rowsToAoa, uniqueSheetName, listDataTables, buildDataTablesWorkbookSpec, DATA_TABLE_LABELS,
} from '../js/data-tables-export.js?v=485';
import { resolveDownloadFilename } from '../js/download.js?v=485';
import {
...
} from '../js/permissions.js?v=485';
import { lotTraceEmptyHint } from '../js/lot-trace.js?v=485';
import {
...
import {
  ...
  FINANCE_ENCODINGS,
} from '../js/finance-import.js?v=485';
...
export async function runAllTests() {
  ...
  test('sanitizeMaterialBarcodes / getMaterialBarcodes — כמה ברקודים לחומר', () => {
    assertEqual(sanitizeBarcode('  7290001  '), '7290001');
    const codes = sanitizeMaterialBarcodes(['7290001', '7290001', '  7290002 ', '', '7290002']);
    assertEqual(codes.join(','), '7290001,7290002');
    const mat = { barcode: '111', barcodes: ['222', '111', '333'] };
    assertEqual(getMaterialBarcodes(mat).join(','), '111,222,333');
    const extra = { name: 'קמח', barcode: '111', barcodes: ['999888'] };
    assertOk(materialMatchesSearch(extra, '999'));
    assertOk(!materialMatchesSearch(extra, '8888'));
  });
  ...
  test('inventoryMovementKindLabel', async () => {
    const { inventoryMovementKindLabel, formatWhatsAppGapOrderText } = await import('../js/inventory-db.js?v=485');
  ...
  test('receiveShortageToInventory — דורש מזהה', async () => {
    const { receiveShortageToInventory } = await import('../js/inventory-db.js?v=485');
    let threw = false;
    try {
  ...
  test('previewProductionStockIssue — דורש מספר מנות', async () => {
    const { previewProductionStockIssue, formatProductionIssueConfirm } = await import('../js/inventory-db.js?v=485');
    let threw = false;
    try {
  ...
  test('HACCP — כלי בניית תכנית מיוצאים', async () => {
    const mod = await import('../js/haccp-db.js?v=485');
  ...
  test('getBackupScopeId — מזהה קבוע לשחזור אחרי מחיקה', async () => {
    const { getBackupScopeId, BACKUP_SCOPE_ID } = await import('../js/supabase-backup.js?v=485');
