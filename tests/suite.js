import { test, testAsync, assertEqual, assertOk, assertApprox, flushTests } from './runner.js?v=467';
import {
  isValidISODate, sanitizeQuantity, sanitizeMoney, sanitizeName, sanitizeRecipeQuantity, roundMoney,
} from '../js/validators.js?v=467';
import {
...
} from '../js/calc.js?v=467';
import { parseDate, parseQuantity, detectAndParse, parseImportFile } from '../js/import.js?v=467';
import { enrichBackupData, summarizeBackupData, formatBackupSummary } from '../js/backup.js?v=467';
import {
...
} from '../js/supplier-import.js?v=467';
import { parseRecipesFromDocumentXml } from '../js/recipe-import.js?v=467';
import { isFlowsReportType, isManagerReportType, normalizeReportType, groupRunsByFlow, filterProductionHistoryEntries, productIdsForHistoryScope, sortProductionHistoryEntries, managerRecordInDateRange, filterManagerTasksByRange, buildPnlDailyRows, sumProcessSummaryQty } from '../js/screens/reports.js?v=467';
import { runStepsAllCompleted, findNextIncompleteStepIndex, parseNumericBatchNumber, computeNextBatchNumber } from '../js/db.js?v=467';
import { haccpRoleLabel, HACCP_STEPS, evaluateCcpDecisionTree, formatCriticalLimit, haccpMonitorMethodLabel, haccpMonitorFrequencyLabel, haccpProductDispositionLabel, haccpVerificationMethodLabel, haccpVerificationFrequencyLabel, haccpDocKindLabel, haccpDocFormatLabel, haccpPrpTopicLabel, haccpPrpStatusLabel, HACCP_PRP_TOPICS, haccpMonitorLogResultLabel, buildHaccpTeamRoleCoverage } from '../js/haccp-db.js?v=467';
import { buildHaccpPlanPrintHtml } from '../js/haccp-print.js?v=467';
import { WORKSPACES } from '../js/workspaces.js?v=467';
import { userRoleLabel, userStatusLabel } from '../js/auth.js?v=467';
import { sanitizeAuditPayload, auditActionLabel, auditEntityLabel, formatAuditSnapshotSummary } from '../js/audit.js?v=467';
import {
...
} from '../js/permissions.js?v=467';
import { lotTraceEmptyHint } from '../js/lot-trace.js?v=467';
...
export async function runAllTests() {
  ...
  test('getSupplierBrowseSyncHint — מזהה מצב «רק ניקיון»', () => {
    const layout = {
      allMaterials: [{ id: 1 }, { id: 2 }, { id: 3 }, { id: 4 }, { id: 5 }],
      categories: [
        {
          id: 1, name: 'חומרי ניקיון', isCleaning: true,
          suppliers: [{ id: 1, materials: [{ id: 1 }] }],
          unassignedMaterials: [],
        },
        {
          id: 2, name: 'חומרי גלם', isCleaning: false, isPackaging: false,
          suppliers: [],
          unassignedMaterials: [],
        },
        {
          id: 3, name: 'אריזות', isPackaging: true,
          suppliers: [],
          unassignedMaterials: [],
        },
      ],
    };
    const hint = getSupplierBrowseSyncHint(layout);
    assertOk(hint.show);
    assertOk(hint.onlyCleaningVisible);
  });
  ...
  test('inventoryMovementKindLabel', async () => {
    const { inventoryMovementKindLabel, formatWhatsAppGapOrderText } = await import('../js/inventory-db.js?v=467');
  ...
  test('receiveShortageToInventory — דורש מזהה', async () => {
    const { receiveShortageToInventory } = await import('../js/inventory-db.js?v=467');
    let threw = false;
    try {
  ...
  test('previewProductionStockIssue — דורש מספר מנות', async () => {
    const { previewProductionStockIssue, formatProductionIssueConfirm } = await import('../js/inventory-db.js?v=467');
    let threw = false;
    try {
  ...
  test('HACCP — כלי בניית תכנית מיוצאים', async () => {
    const mod = await import('../js/haccp-db.js?v=467');
  ...
  test('getBackupScopeId — מזהה קבוע לשחזור אחרי מחיקה', async () => {
    const { getBackupScopeId, BACKUP_SCOPE_ID } = await import('../js/supabase-backup.js?v=467');
