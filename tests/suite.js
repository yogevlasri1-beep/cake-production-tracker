import { test, testAsync, assertEqual, assertOk, assertApprox, flushTests } from './runner.js?v=477';
import {
  isValidISODate, sanitizeQuantity, sanitizeMoney, sanitizeName, sanitizeRecipeQuantity, roundMoney,
} from '../js/validators.js?v=477';
import {
...
} from '../js/calc.js?v=477';
import { parseDate, parseQuantity, detectAndParse, parseImportFile } from '../js/import.js?v=477';
import { enrichBackupData, summarizeBackupData, formatBackupSummary } from '../js/backup.js?v=477';
import {
...
} from '../js/supplier-import.js?v=477';
import { parseRecipesFromDocumentXml } from '../js/recipe-import.js?v=477';
import { isFlowsReportType, isManagerReportType, normalizeReportType, groupRunsByFlow, filterProductionHistoryEntries, productIdsForHistoryScope, sortProductionHistoryEntries, managerRecordInDateRange, filterManagerTasksByRange, buildPnlDailyRows, sumProcessSummaryQty } from '../js/screens/reports.js?v=477';
import { runStepsAllCompleted, findNextIncompleteStepIndex, parseNumericBatchNumber, computeNextBatchNumber, isProductWasteCategory, isWasteProductionEntry, computeRunMetrics, PRODUCT_WASTE_CATEGORY_NAME } from '../js/db.js?v=477';
import { haccpRoleLabel, HACCP_STEPS, evaluateCcpDecisionTree, formatCriticalLimit, haccpMonitorMethodLabel, haccpMonitorFrequencyLabel, haccpProductDispositionLabel, haccpVerificationMethodLabel, haccpVerificationFrequencyLabel, haccpDocKindLabel, haccpDocFormatLabel, haccpPrpTopicLabel, haccpPrpStatusLabel, HACCP_PRP_TOPICS, haccpMonitorLogResultLabel, buildHaccpTeamRoleCoverage } from '../js/haccp-db.js?v=477';
import { buildHaccpPlanPrintHtml } from '../js/haccp-print.js?v=477';
import { WORKSPACES } from '../js/workspaces.js?v=477';
import { userRoleLabel, userStatusLabel } from '../js/auth.js?v=477';
import { sanitizeAuditPayload, auditActionLabel, auditEntityLabel, formatAuditSnapshotSummary } from '../js/audit.js?v=477';
import {
  serializeCell, rowsToAoa, uniqueSheetName, listDataTables, buildDataTablesWorkbookSpec, DATA_TABLE_LABELS,
} from '../js/data-tables-export.js?v=477';
import { resolveDownloadFilename } from '../js/download.js?v=477';
import {
...
export async function runAllTests() {
  ...
  test('inventoryMovementKindLabel', async () => {
    const { inventoryMovementKindLabel, formatWhatsAppGapOrderText } = await import('../js/inventory-db.js?v=477');
  ...
  test('receiveShortageToInventory — דורש מזהה', async () => {
    const { receiveShortageToInventory } = await import('../js/inventory-db.js?v=477');
    let threw = false;
    try {
  ...
  test('previewProductionStockIssue — דורש מספר מנות', async () => {
    const { previewProductionStockIssue, formatProductionIssueConfirm } = await import('../js/inventory-db.js?v=477');
    let threw = false;
    try {
  ...
  test('HACCP — כלי בניית תכנית מיוצאים', async () => {
    const mod = await import('../js/haccp-db.js?v=477');
  ...
  test('getBackupScopeId — מזהה קבוע לשחזור אחרי מחיקה', async () => {
    const { getBackupScopeId, BACKUP_SCOPE_ID } = await import('../js/supabase-backup.js?v=477');
